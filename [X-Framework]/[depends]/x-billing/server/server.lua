...
XCore.Commands.Add("createbill", "Create a bill to send to another player", {{name="id", help="Player ID"},{name="ammount", help="Value of the bill"},{name="reason", help="Reason for the bill"}}, false, function(source, args)
    Player = XCore.Functions.GetPlayer(source)
    OtherPlayer = XCore.Functions.GetPlayer(tonumber(args[1]))
    if Player ~= nil then
...
AddEventHandler('billing:server:PayBill',function(data)
    Player = XCore.Functions.GetPlayer(source)
    OtherPlayer = XCore.Functions.GetPlayerByCitizenId(data[2])
    if Player ~= nil then
       ...
       if Balance - data[1] >= 0 then
            ...
            if OtherPlayer ~= nil then
                ...
                TriggerClientEvent("XCore:NotifyNotify",OtherPlayer.PlayerData.source, "You recieved $"..data[1].." from "..Player.PlayerData.charinfo.firstname.. " ".. Player.PlayerData.charinfo.lastname , "success")
            end
       end
    end
end)

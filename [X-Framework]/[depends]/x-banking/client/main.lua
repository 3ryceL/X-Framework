...
RegisterNetEvent('x-banking:client:syncBanks')
AddEventHandler('x-banking:client:syncBanks', function(data)
    banks = data
    if showing then
        showing = false
    end
...
RegisterNetEvent('x-banking:updateCash')
AddEventHandler('x-banking:updateCash', function(data)
...
function openAccountScreen()
    XCore.Functions.TriggerCallback('x-banking:getBankingInformation', function(banking)
        if banking ~= nil then
            InBank = true
...
function atmRefresh()
    XCore.Functions.TriggerCallback('x-banking:getBankingInformation', function(infor)
        InBank = true
        SetNuiFocus(true, true)
        SendNUIMessage({
            status = "refreshatm",
...
RegisterNetEvent('x-banking:openBankScreen')
AddEventHandler('x-banking:openBankScreen', function()
    openAccountScreen()
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        letSleep = true
        if playerLoaded and XCore ~= nil and not InBank then 
...
RegisterNetEvent('x-banking:transferError')
AddEventHandler('x-banking:transferError', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('x-banking:successAlert')
AddEventHandler('x-banking:successAlert', function(msg)
    SendNUIMessage({

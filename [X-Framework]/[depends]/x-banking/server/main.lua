Citizen.CreateThread(function()
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Business'", function(accts)
        buis = #accts
        if accts[1] ~= nil then
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Savings'", function(savings)
        sav = #savings
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_type` = 'Gang'", function(gangs)
        gang = #gangs
        if gangs[1] ~= nil then
...
RegisterServerEvent('x-banking:server:modifyBank')
AddEventHandler('x-banking:server:modifyBank', function(bank, k, v)
    if banks[tonumber(bank)] then
        banks[tonumber(bank)][k] = v
        TriggerClientEvent('x-banking:client:syncBanks', -1, banks)
    end
end)

exports('modifyBank', function(bank, k, v)
    TriggerEvent('x-banking:server:modifyBank', bank, k, v)
end)
...
function checkAccountExists(acct, sc)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_number` = '" .. acct .. "' AND `sort_code` = '" .. sc .. "'", function(exists)
...
RegisterServerEvent('x-banking:createNewCard')
AddEventHandler('x-banking:createNewCard', function()
    local src = source
    ...
    TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', "Created new card **[" .. xPlayer.PlayerData.citizenid .. "]**", src)
end)

RegisterServerEvent('x-base:itemUsed')
AddEventHandler('x-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('x-banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('x-banking:server:unpackMoneyBag')
AddEventHandler('x-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
        local xPlayer = XCore.Functions.GetPlayer(_src)
...
RegisterServerEvent('x-banking:initiateTransfer')
AddEventHandler('x-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    local _startChar = XCore.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then 
        ...
        if receiptName ~= false or receiptName ~= nil then 
            local userOnline = exports.x-base:checkOnline(cid)
            
            if userOnline ~= false then
                -- User is online so we can do a straght transfer 
                local _targetUser = exports.x-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('x-banking:openBankScreen', _src)
                    TriggerClientEvent('x-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('x-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
XCore.Functions.CreateCallback('x-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-banking:createBankCard')
AddEventHandler('x-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('x-banking:openBankScreen', src)
    TriggerClientEvent('XCore:NotifyNotify', src, 'You have successfully ordered a Debit Card.', 'success')
    
    TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', 'Successfully ordered a Debit Card', src)
end)

RegisterServerEvent('x-banking:doQuickDeposit')
AddEventHandler('x-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('x-banking:openBankScreen', src)
            TriggerClientEvent('x-banking:successAlert', src, 'You made a cash deposit of $'..amount..' successfully.')
            TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', 'Made a cash deposit of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('x-banking:toggleCard')
AddEventHandler('x-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-banking:doQuickWithdraw')
AddEventHandler('x-banking:doQuickWithdraw', function(amount, branch)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('x-banking:openBankScreen', src)
            TriggerClientEvent('x-banking:successAlert', src, 'You made a cash withdrawal of $'..amount..' successfully.')
            TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', 'Made a cash withdrawal of $'..amount..' successfully.', src)
        end
    end
end)

RegisterServerEvent('x-banking:updatePin')
AddEventHandler('x-banking:updatePin', function(pin)
    if pin ~= nil then 
        local src = source
        local xPlayer = XCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('x-banking:openBankScreen', src)
        TriggerClientEvent('x-banking:successAlert', src, 'You have successfully updated your Debit card pin.')
    end
end)

RegisterServerEvent('x-banking:savingsDeposit')
AddEventHandler('x-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('x-banking:openBankScreen', src)
        TriggerClientEvent('x-banking:successAlert', src, 'You made a savings deposit of $'..tostring(amount)..' successfully.')
        TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', 'made a savings deposit of $'..tostring(amount)..' successfully..', src)
    end
end)

RegisterServerEvent('x-banking:savingsWithdraw')
AddEventHandler('x-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('x-banking:openBankScreen', src)
        TriggerClientEvent('x-banking:successAlert', src, 'You made a savings withdrawal of $'..tostring(amount)..' successfully.')
        TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', 'Made a savings withdrawal of $'..tostring(amount)..' successfully.', src)
    end
end)

RegisterServerEvent('x-banking:createSavingsAccount')
AddEventHandler('x-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('x-banking:openBankScreen', src)
    TriggerClientEvent('x-banking:successAlert', src, 'You have successfully opened a savings account.')
    TriggerEvent('x-log:server:CreateLog', 'banking', 'Banking', "Created new saving account", src)
end)

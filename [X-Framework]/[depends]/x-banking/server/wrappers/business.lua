...
function generatebusinessAccount(acc, sc, bid)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `account_number` = @acc AND `sort_code` = @sc AND `businessid` = @bid", {['@acc'] = self.accountNumber, ['@sc'] = self.sortCode, ['@bid'] = self.bid}, function(bankAccount)
    ...
    rTable.getBankStatement = function(limit)
        ...
        local processed = false
        local statement
        XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_statements` WHERE `account` = 'business' AND `business` = @bus AND `account_number` = @acc AND `sort_code` = @sc AND `businessid` = @bid LIMIT @limit", {['@bus'] = bankAccount[1].business, ['@acc'] = self.accountNumber, ['@sc'] = self.sortCode, ['@limit'] = resLimit, ['@bid'] = self.bid }, function(res)
            statement = res
            processed = true
        end)
        repeat Wait(0) until processed == true
...
function createbusinessAccount(accttype, bid, startingBalance)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `business` = @accttype AND `businessid` = @bid", {['@accttype'] = accttype, ['@bid'] = bid}, function(checkExists)

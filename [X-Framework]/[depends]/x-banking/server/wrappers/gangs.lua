function loadGangAccount(gangid)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `gangid` = @gang AND `account_type` = 'Gang'", {['@gang'] = self.gid}, function(query)
        if query[1] ~= nil then
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_statements` WHERE `account_number` = @ac AND `sort_code` = @sc AND `gangid` = @gid", {['@ac'] = self.accountnumber, ['@sc'] = self.sortcode, ['@gid'] = self.gid}, function(state)
...
function createGangAccount(gang, startingBalance)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `gangid` = @gang", {['@gang'] = gang}, function(checkExists)

function generateCurrent(cid)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_cards` WHERE `citizenid` = '" .. self.cid .."'", function(getCurrentAccount)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_statements` WHERE `account` = 'Current' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(bankStatement)
    ...
    self.updateItemPin = function(pin)
        local processed = false
        local success
        XCore.Functions.ExecuteSql(true, "SELECT * FROM `stored_items` WHERE `metaprivate` LIKE '%\"cardnumber\":"..self.cardNumber.."%' AND `metaprivate` LIKE '%\"account\":"..self.account.."%' AND `metaprivate` LIKE '%\"sortcode\":"..self.sortcode.."%' AND `type` = 'Bankcard' LIMIT 1", function(item)
            if item[1] ~= nil then
                itemFound = true
        ...
        if not self.cardActive then
            ...
            exports.ghmattimysql:execute('UPDATE bank_cards SET cardnumber=@cardnumber, cardPin=@cardPin, cardDecrypted=@cardDecrypted, cardActive=@cardActive, cardLocked=@cardLocked, cardType=@cardType WHERE citizenid=@citizenid AND record_id=@record_id', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('x-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = XCore.Functions.GetPlayer(self.source)
...
RegisterServerEvent('x-banking:server:registerCurrentAccount')
AddEventHandler('x-banking:server:registerCurrentAccount', function(cid)
...
function generateSavings(cid)
    ...
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_accounts` WHERE `citizenid` = '" .. self.cid .."' AND `account_type` = 'Savings'", function(getSavingsAccount)
        if getSavingsAccount[1] ~= nil then
            self.aid = getSavingsAccount[1].record_id
            self.balance = getSavingsAccount[1].amount
        end
        
        XCore.Functions.ExecuteSql(true, "SELECT * FROM `bank_statements` WHERE `account` = 'Savings' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(stats)
...
RegisterServerEvent('x-banking:server:registerSavingsAccount')
AddEventHandler('x-banking:server:registerSavingsAccount', function(cid)

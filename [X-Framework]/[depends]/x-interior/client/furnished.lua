...
RegisterNetEvent('x-interior:client:SetNewState')
AddEventHandler('x-interior:client:SetNewState', function(bool)
	IsNew = bool
end)
...
function CreateApartmentFurnished(spawn)
	...
	if IsNew then
		SetTimeout(750, function()
			TriggerEvent('x-clothes:client:CreateFirstCharacter')
			IsNew = false
		end)
	end
    return { objects, POIOffsets }

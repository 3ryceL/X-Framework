...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('x-weathersync:server:RequestStateSync')
    TriggerServerEvent('x-weathersync:server:RequestCommands')
end)

RegisterNetEvent('x-weathersync:client:EnableSync')
AddEventHandler('x-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('x-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('x-weathersync:client:DisableSync')
AddEventHandler('x-weathersync:client:DisableSync', function()
	disable = true
...
RegisterNetEvent('x-weathersync:client:SyncWeather')
AddEventHandler('x-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('x-weathersync:client:RequestCommands')
AddEventHandler('x-weathersync:client:RequestCommands', function(isAllowed)
    if isAllowed then
...
RegisterNetEvent('x-weathersync:client:SyncTime')
AddEventHandler('x-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)

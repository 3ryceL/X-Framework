...
XShared.RandomStr = function(length) -- XShared.RandomStr falls under GPL License here: [esxlicense]/LICENSE
	if length > 0 then
		return XShared.RandomStr(length-1) .. StringCharset[math.random(1, #StringCharset)]
	else
		return ''
	end
end

XShared.RandomInt = function(length)
	if length > 0 then
		return XShared.RandomInt(length-1) .. NumberCharset[math.random(1, #NumberCharset)]
	else
		return ''
	end
end

XShared.SplitStr = function(str, delimiter)
	local result = { }
	local from  = 1

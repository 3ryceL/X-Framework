    ...
    while true do
        ...
        if IsControlJustReleased(0, 47) then 
            if not harnessOn then
                ...
                XCore.Functions.Progressbar("harness_equip", "Race harnas af doen..", 5000, false, true, {
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            XCore.Functions.Progressbar("harness_equip", "Race harnas om doen..", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        XCore.Functions.Notify('You\'re not in a car.', 'error', 3500)
    end
end)

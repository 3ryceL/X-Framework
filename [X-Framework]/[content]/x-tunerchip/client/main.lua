...
RegisterNUICallback('save', function(data)
    XCore.Functions.TriggerCallback('x-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            XCore.Functions.Notify('TunerChip v1.05: Vehicle Tuned!', 'error')

            TriggerServerEvent('x-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('x-tunerchip:server:TuneStatus')
AddEventHandler('x-tunerchip:server:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    XCore.Functions.TriggerCallback('x-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    XCore.Functions.Notify('TunerChip v1.05: Vehicle has been reset!', 'error')
end)

RegisterNetEvent('x-tunerchip:client:openChip')
AddEventHandler('x-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        XCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Vehicle Has Been Reset!", 2000, false, true, {

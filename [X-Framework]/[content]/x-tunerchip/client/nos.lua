...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    XCore.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            XCore.Functions.Progressbar("use_nos", "NOS Is Connected", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", XCore.Shared.Items['nitrous'], "remove")
                TriggerServerEvent("XCore:NotifyServer:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            XCore.Functions.Notify('You\'re Not In A Car', 'error')
        end
    else
        XCore.Functions.Notify('You Already Have NOS Active', 'error')
    end
end)

...
$(document).on('click', '#save', function(){
    $.post('https://x-tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://x-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://x-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://x-tunerchip/SetStancer', JSON.stringify({
...
XTuner.Open = function() {
    $.post('https://x-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
XTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('https://x-tunerchip/exit');
}
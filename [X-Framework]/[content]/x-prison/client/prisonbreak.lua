...
Citizen.CreateThread(function()
    ...
    requiredItems = {
        ...
        [2] = {name = XCore.Shared.Items["gatecrack"]["name"], image = XCore.Shared.Items["gatecrack"]["image"]},
    }
    while true do 
        Citizen.Wait(5)
        inRange = false
...
Citizen.CreateThread(function()
	while true do
        ...
        if #(pos - vector3(Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z)) > 200 and inJail then
            ...
            XCore.Functions.Notify("You escaped... Get the hell out of here.!", "error")
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then 
        XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(result)
            if result then 
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                XCore.Functions.Progressbar("hack_gate", "Electronic kit plug in..", math.random(5000, 10000), false, true, {
                    ...
                    StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                    XCore.Functions.Notify("Cancelled..", "error")
                end)
            else
...
function OnHackDone(success, timeremaining)
    if success then
		...
		TriggerServerEvent('x-doorlock:server:updateState', Gates[currentGate].gatekey, false)

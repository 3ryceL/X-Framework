...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
	...
	XCore.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	while true do 
		...
		if jailTime > 0 and inJail then 
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					XCore.Functions.Notify("Your time is up! Check yourself out at the visitors center", "success", 10000)
				end
...
AddEventHandler('prison:client:Enter', function(time)
	XCore.Functions.Notify("You're in jail for "..time.." months..", "error")
	...
	DoScreenFadeIn(1000)
	XCore.Functions.Notify("Do some work for sentence reduction, instant job: "..Config.Jobs[currentJob])
end)

RegisterNetEvent('prison:client:Leave')
AddEventHandler('prison:client:Leave', function()
	if jailTime > 0 then 
		XCore.Functions.Notify("You still have to... "..jailTime.." months..")
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		XCore.Functions.Notify("You're free! Enjoy it! :)")
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Citizen.Wait(10)
		end

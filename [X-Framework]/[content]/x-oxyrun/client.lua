...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			XCore.Functions.Notify('The delivery was on point, your GPS will be updated with the next drop off', 'success')
		else
			XCore.Functions.Notify('The Drop Off Failed', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			XCore.Functions.Notify('You Are Close To The Drop Off Point')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			XCore.Functions.Notify('You Took Too Long', 'error')
		end
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				XCore.Functions.Notify('The Car Is Too Damaged', 'error')
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
	    	...
	    	for i = 1, #drugLocs do
				...
				if dstcheck < 5.0 then
					...
					local job = XCore.Functions.GetPlayerData().job.name
					...
					if job ~= "police" then
				    	...
				    	if IsControlJustReleased(0,38) then
							...
							if not daytime then
								XCore.Functions.Notify('Come Back During The Day', 'error')
		            		else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	firstdeal = true
	CreateOxyVehicle()
	XCore.Functions.Notify('A Car Has Been Provided. Your GPS Will Be Updated With Locations Soon', 'success')
end)

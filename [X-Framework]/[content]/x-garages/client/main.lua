...
--     XCore.Functions.TriggerCallback('x-garage:server:GetOutsideVehicles', function(result)
--         if result ~= nil then
...
AddEventHandler('XCore:NotifyClient:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('x-garages:client:setHouseGarage')
AddEventHandler('x-garages:client:setHouseGarage', function(house, hasKey)
    currentHouseGarage = house
    hasGarageKey = hasKey
end)

RegisterNetEvent('x-garages:client:houseGarageConfig')
AddEventHandler('x-garages:client:houseGarageConfig', function(garageConfig)
    HouseGarages = garageConfig
end)

RegisterNetEvent('x-garages:client:addHouseGarage')
AddEventHandler('x-garages:client:addHouseGarage', function(house, garageInfo)
    HouseGarages[house] = garageInfo
end)

-- function AddOutsideVehicle(plate, veh)
--     OutsideVehicles[plate] = veh
--     TriggerServerEvent('x-garages:server:UpdateOutsideVehicles', OutsideVehicles)
-- end

RegisterNetEvent('x-garages:client:takeOutDepot')
AddEventHandler('x-garages:client:takeOutDepot', function(vehicle)
    if OutsideVehicles ~= nil and next(OutsideVehicles) ~= nil then
        if OutsideVehicles[vehicle.plate] ~= nil then
            ...
            -- if Engine <= 50.0 then
                XCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                    XCore.Functions.TriggerCallback('x-garage:server:GetVehicleProperties', function(properties)
                        XCore.Functions.SetVehicleProperties(veh, properties)
                        ...
                        TriggerServerEvent('x-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                        XCore.Functions.Notify("Vehicle Off:Engine " .. enginePercent .. "% Body: " .. bodyPercent.. "% Fuel: "..currentFuel.. "%", "primary", 4500)
            ...
            -- else
            --     XCore.Functions.Notify("You cannot duplicate this vehicle")
            -- end
        else
            XCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                XCore.Functions.TriggerCallback('x-garage:server:GetVehicleProperties', function(properties)
...
function HouseGarage(house)
    XCore.Functions.TriggerCallback("x-garage:server:GetHouseVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Depot Vehicles :"
        ClearMenu()

        if result == nil then
            XCore.Functions.Notify("You have no vehicles in your garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                if v.vehicle ~= "yFiat595ssB" then
                    Menu.addButton(XCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGarageVehicle", v, v.state, " Motor: " .. enginePercent.."%", " Body: " .. bodyPercent.."%", " Fuel: "..currentFuel.."%")
                else
...
function DepotList()
    XCore.Functions.TriggerCallback("x-garage:server:GetDepotVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Impounded Vehicles :"
        ClearMenu()

        if result == nil then
            XCore.Functions.Notify("There are no vehicles in the Impound", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                if v.vehicle ~= "yFiat595ssB" then
                    Menu.addButton(XCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutDepotVehicle", v, v.state .. " ($"..v.depotprice..",-)", " Motor: " .. enginePercent.."%", " Body: " .. bodyPercent.."%", " Fuel: "..currentFuel.."%")
                else
...
function VehicleList()
    XCore.Functions.TriggerCallback("x-garage:server:GetUserVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "My Vehicles :"
        ClearMenu()

        if result == nil then
            XCore.Functions.Notify("You have no vehicles in this garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                if v.vehicle ~= "yFiat595ssB" then
                    Menu.addButton(XCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutVehicle", v, v.state, " Motor: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
                else
    ...
    XCore.Functions.TriggerCallback("x-garage:server:GetUserVehicles", function(result)
        ...
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                if v.vehicle ~= "yFiat595ssB" then
                    Menu.addButton(XCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGangVehicle", v, v.state, " Motor: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
                else
...
function TakeOutVehicle(vehicle)
    ...
    elseif vehicle.state == "Out" then
        XCore.Functions.Notify("Is your vehicle in the Depot", "error", 2500)
    elseif vehicle.state == "Impound" then
        XCore.Functions.Notify("This vehicle was impounded by the Police", "error", 4000)
    end
end
...
function TakeOutDepotVehicle(vehicle)
    if vehicle.state == "Impound" then
        TriggerServerEvent("x-garage:server:PayDepotPrice", vehicle)
    end
end
            ...
            if putDist <= 25 and IsPedInAnyVehicle(ped) then
                ...
                if putDist <= 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                local bodyDamage = math.ceil(GetVehicleBodyHealth(curVeh))
                                local engineDamage = math.ceil(GetVehicleEngineHealth(curVeh))
                                local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)

                                TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, k)
                                TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, k)
                                ...
                                XCore.Functions.Notify("Vehicle Parked In, "..Garages[k].label, "primary", 4500)
                            else
                                XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                            end
                        end, plate)
                    end
                end
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, "ballasgarage")
                                    TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, "ballasgarage")
                                    --TriggerServerEvent('vehiclemod:server:saveStatus', plate)
                                    XCore.Functions.DeleteVehicle(curVeh)
                                    if plate ~= nil then
                                        OutsideVehicles[plate] = veh
                                    ...
                                    XCore.Functions.Notify("Vehicle Parked In, "..GangGarages["ballasgarage"].label, "primary", 4500)
                                else
                                    XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                end
                            end, plate)
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, "la_familiagarage")
                                    TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, "la_familiagarage")
                                    --TriggerServerEvent('vehiclemod:server:saveStatus', plate)
                                    XCore.Functions.DeleteVehicle(curVeh)
                                    if plate ~= nil then
                                        OutsideVehicles[plate] = veh
                                    ...
                                    XCore.Functions.Notify("Vehicle Parked In, "..GangGarages["la_familiagarage"].label, "primary", 4500)
                                else
                                    XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                end
                            end, plate)
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, "nomadsgarage")
                                    TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, "nomadsgarage")
                                    --TriggerServerEvent('vehiclemod:server:saveStatus', plate)
                                    XCore.Functions.DeleteVehicle(curVeh)
                                    if plate ~= nil then
                                        OutsideVehicles[plate] = veh
                                    ...
                                    XCore.Functions.Notify("Vehicle Parked In, "..GangGarages["nomadsgarage"].label, "primary", 4500)
                                else
                                    XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                end
                            end, plate)
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, "cartelgarage")
                                    TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, "cartelgarage")
                                    --TriggerServerEvent('vehiclemod:server:saveStatus', plate)
                                    XCore.Functions.DeleteVehicle(curVeh)
                                    if plate ~= nil then
                                        OutsideVehicles[plate] = veh
                                    ...
                                    XCore.Functions.Notify("Vehicle Parked In, "..GangGarages["cartelgarage"].label, "primary", 4500)
                                else
                                    XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                end
                            end, plate)
                    ...
                    if takeDist < 2.0 then
                        ...
                        elseif IsPedInAnyVehicle(ped) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                local curVeh = GetVehiclePedIsIn(ped)
                                local plate = GetVehicleNumberPlateText(curVeh)
                                XCore.Functions.TriggerCallback('x-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        local bodyDamage = round(GetVehicleBodyHealth(curVeh), 1)
                                        local engineDamage = round(GetVehicleEngineHealth(curVeh), 1)
                                        local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)

                                        TriggerServerEvent('x-garage:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, currentHouseGarage)
                                        TriggerServerEvent('x-garage:server:updateVehicleState', 1, plate, currentHouseGarage)
                                        XCore.Functions.DeleteVehicle(curVeh)
                                        if plate ~= nil then
                                            OutsideVehicles[plate] = veh
                                            TriggerServerEvent('x-garages:server:UpdateOutsideVehicles', OutsideVehicles)
                                        end
                                        XCore.Functions.Notify("Vehicle Parked In, "..HouseGarages[currentHouseGarage], "primary", 4500)
                                    else
                                        XCore.Functions.Notify("Nobody owns this vehicle", "error", 3500)
                                    end

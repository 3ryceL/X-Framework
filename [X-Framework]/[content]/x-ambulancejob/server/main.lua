...
AddEventHandler('hospital:server:RespawnAtHospital', function()
	...
	for k, v in pairs(Config.Locations["beds"]) do
		...
		TriggerClientEvent('XCore:NotifyNotify', src, 'All your possessions have been taken..', 'error')
...
AddEventHandler('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = XCore.Functions.GetPlayer(src)
	local Patient = XCore.Functions.GetPlayer(playerId)
	if Patient ~= nil then
		if Player.PlayerData.job.name =="ambulance" then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, XCore.Shared.Items['bandage'], "remove")
	...
	if Patient ~= nil then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', source, XCore.Shared.Items['firstaid'], "remove")
				TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
			else
				TriggerClientEvent('XCore:NotifyNotify', src, "You don\'t have enough money on you..", "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', source, XCore.Shared.Items['firstaid'], "remove")
...
AddEventHandler('hospital:server:MakeDeadCall', function(blipSettings, gender, street1, street2)
	...
	if street2 ~= nil then
		...
		TriggerClientEvent('x-policealerts:client:AddPoliceAlert', -1, {
            timeOut = 5000,
...
XCore.Functions.CreateCallback('hospital:GetDoctors', function(source, cb)
	local amount = 0
	for k, v in pairs(XCore.Functions.GetPlayers()) do
		local Player = XCore.Functions.GetPlayer(v)
		if Player ~= nil then 
...
XCore.Functions.CreateCallback('hospital:GetPlayerStatus', function(source, cb, playerId)
...
XCore.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
XCore.Commands.Add("status", "Check A Players Health", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", source)
	else
		TriggerClientEvent('XCore:NotifyNotify', source, "You Are Not EMS", "error")
	end
end)

XCore.Commands.Add("heal", "Heal A Player", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source) 
	if Player.PlayerData.job.name == "ambulance" then
...
XCore.Commands.Add("revivep", "Revive A Player", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
...
XCore.Commands.Add("revive", "Revive A Player or Yourself (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
...
XCore.Commands.Add("setpain", "Set Yours or A Players Pain Level (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = XCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
XCore.Commands.Add("kill", "Kill A Player or Yourself (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = XCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
XCore.Functions.CreateUseableItem("bandage", function(source, item)
	local Player = XCore.Functions.GetPlayer(source)
...
XCore.Functions.CreateCallback('hospital:server:HasBandage', function(source, cb)
    local player = XCore.Functions.GetPlayer(source)
...
XCore.Functions.CreateUseableItem("painkillers", function(source, item)
	local Player = XCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
XCore.Functions.CreateUseableItem("firstaid", function(source, item)
	local Player = XCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
XCore.Functions.CreateCallback('hospital:server:HasFirstAid', function(source, cb)
    local player = XCore.Functions.GetPlayer(source)
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('XCore:NotifyNotify', helperId, "You can\'t help this person..", "error")
	end
end)
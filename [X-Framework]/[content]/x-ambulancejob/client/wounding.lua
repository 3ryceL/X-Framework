            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", XCore.Functions.GetPlayerData().citizenid, XCore.Functions.GetPlayerData().metadata["bloodtype"], coords)
...
AddEventHandler('hospital:client:UseBandage', function()
    ...
    XCore.Functions.Progressbar("use_bandage", "Using bandage..", 4000, false, true, {
        ...
        TriggerServerEvent("XCore:NotifyServer:RemoveItem", "bandage", 1)
        TriggerEvent("inventory:client:ItemBox", XCore.Shared.Items["bandage"], "remove")
...
AddEventHandler('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    XCore.Functions.Progressbar("use_bandage", "Taking painkillers", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("XCore:NotifyServer:RemoveItem", "painkillers", 1)
        TriggerEvent("inventory:client:ItemBox", XCore.Shared.Items["painkillers"], "remove")
        onPainKillers = true

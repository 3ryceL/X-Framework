        ...
        if XCore ~= nil then
            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                if doctorCount >= Config.MinimalDoctors then
                    XCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Call doctor")
                else
                    XCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Check in")
                end
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        XCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                XCore.Functions.Notify("Beds are occupied..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            XCore.Functions.Notify("Checking in failed!", "error")
                        end)
                    end
                end
            elseif #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 4.5 then
                if doctorCount >= Config.MinimalDoctors then
                    XCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Call")
                else
                    XCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Check in")
                end
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if #(pos - vector3(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z)) < 1.5 then
                    XCore.Functions.DrawText3D(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z + 0.3, "~g~E~w~ - To lie in bed")
                    if IsControlJustReleased(0, 38) then
    ...
    while true do
        ...
        if XCore ~= nil then
            if isInHospitalBed and canLeaveBed then
                local pos = GetEntityCoords(PlayerPedId())
                XCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - To get out of bed..")
                if IsControlJustReleased(0, 38) then
                    LeaveBed()
                end
            else
...
AddEventHandler('hospital:client:Revive', function()
    ...
    XCore.Functions.Notify("You are completely healthy again!")
end)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    XCore.Functions.Notify("Your wounds have been healed!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        Citizen.Wait(5)
        if isRevive then
            XCore.Functions.Notify("You are being helped..")
...
function DoLimbAlert()
    if not isDead then
        if #injured > 0 then
            ...
            XCore.Functions.Notify(limbDamageMsg, "primary", 5000)
        end
    end
end
...
function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        XCore.Functions.Notify("You are "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
function ResetAll()
    ...
    TriggerServerEvent("XCore:NotifyServer:SetMetaData", "hunger", 100)
    TriggerServerEvent("XCore:NotifyServer:SetMetaData", "thirst", 100)
end

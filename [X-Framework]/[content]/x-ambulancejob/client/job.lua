...
AddEventHandler('hospital:client:AiCall', function()
    ...
    local gender = XCore.Functions.GetPlayerData().gender
...
AddEventHandler('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        XCore.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result ~= nil then
                for k, v in pairs(result) do
                    ...
                    elseif result["BLEED"] > 0 then
                        ...
                        XCore.Functions.Notify('Player Is Healthy', 'success')
                    end
                end
...
AddEventHandler('hospital:client:RevivePlayer', function()
    XCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                XCore.Functions.Progressbar("hospital_revive", "Reviving person..", 5000, false, true, {
                    ...
                    XCore.Functions.Notify("You revived the person!")
            ...
            XCore.Functions.Notify("You Need A First Aid Kit", "error")
        end
    end, 'firstaid')
end)
...
AddEventHandler('hospital:client:TreatWounds', function()
    XCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                XCore.Functions.Progressbar("hospital_healwounds", "Healing wounds..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    XCore.Functions.Notify("You helped the person!")
            ...
            XCore.Functions.Notify("You Need A Bandage", "error")
        end
    end, 'bandage')
end)
...
function TakeOutVehicle(vehicleInfo)
    ...
    XCore.Functions.SpawnVehicle(vehicleInfo[1], function(veh)

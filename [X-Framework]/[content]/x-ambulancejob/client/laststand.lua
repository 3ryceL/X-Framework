        ...
        Citizen.CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    XCore.Functions.Notify("You have bled out..", "error")
                    SetLaststand(false)
                    ...
                    local weaponLabel = XCore.Shared.Weapons[killerWeapon] ~= nil and XCore.Shared.Weapons[killerWeapon]["label"] or "Unknown"
                    local weaponName = XCore.Shared.Weapons[killerWeapon] ~= nil and XCore.Shared.Weapons[killerWeapon]["name"] or "Unknown_Weapon"
                    TriggerServerEvent("x-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has killed  ".. GetPlayerName(player) .." with a **".. weaponLabel .. "** (" .. weaponName .. ")")
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
AddEventHandler('hospital:client:UseFirstAid', function()
    if not isEscorting then
        ...
        XCore.Functions.Notify('Action impossible!', 'error')
    end
end)
...
AddEventHandler('hospital:client:HelpPerson', function(targetId)
    ...
    XCore.Functions.Progressbar("hospital_revive", "Reviving person..", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        XCore.Functions.Notify("You revived a person.")

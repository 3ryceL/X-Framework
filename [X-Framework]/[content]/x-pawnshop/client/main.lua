				...
				if GetClockHours() >= 7 and GetClockHours() <= 17 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						if IsControlJustReleased(0, 38) then
                            ...
                            XCore.Functions.Progressbar("sell_pawn_items", "Selling Items", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(PlayerPedId())
								TriggerServerEvent("x-pawnshop:server:sellPawnItems")
								sellItemsSet = false
								sellPrice = 0
                            end, function() -- Cancel
								ClearPedTasks(PlayerPedId())
			...
			if #(pos - Config.PawnHardwareLocation) < 1.5 then
				if GetClockHours() >= 9 and GetClockHours() <= 16 then
					...
					elseif sellHardwareItemsSet and sellHardwarePrice ~= 0 then
						...
						if IsControlJustReleased(0, 38) then
							TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            XCore.Functions.Progressbar("sell_pawn_items", "Sell things", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(PlayerPedId())
								TriggerServerEvent("x-pawnshop:server:sellHardwarePawnItems")
...
function GetSellingPrice()
	local price = 0
	XCore.Functions.TriggerCallback('x-pawnshop:server:getSellPrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
...
function GetSellingHardwarePrice()
	local price = 0
	XCore.Functions.TriggerCallback('x-pawnshop:server:getSellHardwarePrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price

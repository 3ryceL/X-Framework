...
AddEventHandler("XCore:NotifyClient:OnPlayerLoaded", function()
    XCore.Functions.TriggerCallback('x-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("x-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, 38) then 
                            ...
                            ScrapAnim(1000)
                            XCore.Functions.Progressbar("drop_golden_stuff", "Grab Items", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("x-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            XCore.Functions.Progressbar("sell_gold", "Selling Gold", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('x-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
...
function HasPlayerGold()
	local retval = false
	XCore.Functions.TriggerCallback('x-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('x-pawnshop:client:startMelting')
AddEventHandler('x-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('x-pawnshop:client:SetTakeState')
AddEventHandler('x-pawnshop:client:SetTakeState', function(state)

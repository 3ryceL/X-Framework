...
RegisterServerEvent("x-pawnshop:server:sellPawnItems")
AddEventHandler("x-pawnshop:server:sellPawnItems", function()
    local src = source
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('XCore:NotifyNotify', src, "You have sold your items")
    end
end)

RegisterServerEvent("x-pawnshop:server:sellHardwarePawnItems")
AddEventHandler("x-pawnshop:server:sellHardwarePawnItems", function()
    local src = source
    local price = 0
    local Player = XCore.Functions.GetPlayer(src)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
RegisterServerEvent("x-pawnshop:server:getGoldBars")
AddEventHandler("x-pawnshop:server:getGoldBars", function()
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    if GoldBarsAmount > 0 then
        if Player.Functions.AddItem("goldbar", GoldBarsAmount) then
            ...
            Config.MeltTime = 300
            TriggerClientEvent("x-pawnshop:client:SetTakeState", -1, false)
        else
            TriggerClientEvent('XCore:NotifyNotify', src, "You have no space in your inventory", "error")
        end
    end
end)

RegisterServerEvent("x-pawnshop:server:sellGold")
AddEventHandler("x-pawnshop:server:sellGold", function()
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
RegisterServerEvent("x-pawnshop:server:meltItems")
AddEventHandler("x-pawnshop:server:meltItems", function()
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    local goldbars = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('XCore:NotifyNotify', src, "You do not have enough " .. Player.PlayerData.items[k].label, "error")
                    else
        ...
        if goldbars > 0 then
            ...
            TriggerClientEvent('x-pawnshop:client:startMelting', -1)
            ...
            Citizen.CreateThread(function()
                while Config.IsMelting do
                    ...
                    if Config.MeltTime <= 0 then
                        ...
                        Config.CanTake = true
                        Config.MeltTime = 300
                        TriggerClientEvent('x-pawnshop:client:SetTakeState', -1, true)
                    end
                    Citizen.Wait(1000)
                end
            end)
...
XCore.Functions.CreateCallback('x-pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
XCore.Functions.CreateCallback('x-pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

XCore.Functions.CreateCallback('x-pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
...
XCore.Functions.CreateCallback('x-pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = XCore.Functions.GetPlayer(source)

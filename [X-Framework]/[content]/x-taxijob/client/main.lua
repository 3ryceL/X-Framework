...
RegisterNetEvent('x-taxi:client:DoTaxiNpc')
AddEventHandler('x-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                XCore.Functions.Notify('The NPC Is Indicated On Your GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    XCore.Functions.Notify('Bring The NPC To The Specified Location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                XCore.Functions.Notify('You Are Already Doing An NPC Mission')
            end
      --  else
         --   XCore.Functions.Notify('No NPCs Are Available')
       -- end
    else
        XCore.Functions.Notify('You Are Not In A Taxi')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent('x-taxi:server:NpcPay', meterData.currentFare)
                        XCore.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('x-taxi:client:toggleMeter')
AddEventHandler('x-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            XCore.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        XCore.Functions.Notify('You\'re Not In A Vehicle ', 'error')
    end
end)

RegisterNetEvent('x-taxi:client:enableMeter')
AddEventHandler('x-taxi:client:enableMeter', function()
    local ped = PlayerPedId()
    ...
    if meterIsOpen then
        ...
        XCore.Functions.Notify('The Taxi Meter Is Not Active', 'error')
    end
end)
...
RegisterNetEvent('x-taxi:client:toggleMuis')
AddEventHandler('x-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        XCore.Functions.Notify('No Taxi Meter In Sight', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    XCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)

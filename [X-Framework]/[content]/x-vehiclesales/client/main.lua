            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        XCore.Functions.TriggerCallback('x-occasions:server:getVehicles', function(vehicles)
                ...
                if sellBackDist <= 3.5 and IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        XCore.Functions.TriggerCallback('x-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                TriggerServerEvent('x-occasions:server:sellVehicleBack', sellVehData)
                                XCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                            else
                                XCore.Functions.Notify('This is not your vehicle..', 'error', 3500)
                            end
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, XCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Price: ~g~$'..Config.OccasionSlots[i]["price"])
                                if Config.OccasionSlots[i]["owner"] == XCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    XCore.Functions.TriggerCallback('x-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, 161) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("x-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            XCore.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = XCore.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    XCore.Functions.Notify(data.message, 'error')
end)

RegisterNUICallback('buyVehicle', function()
    local vehData = Config.OccasionSlots[currentVehicle]
    TriggerServerEvent('x-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('x-occasions:client:BuyFinished')
AddEventHandler('x-occasions:client:BuyFinished', function(vehdata)
    local vehmods = json.decode(vehdata.mods)

    DoScreenFadeOut(250)
    Citizen.Wait(500)
    XCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        XCore.Functions.Notify("Vehicle Bought", "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        XCore.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('x-occasions:client:ReturnOwnedVehicle')
AddEventHandler('x-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    XCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        XCore.Functions.Notify("You vehicle is returned")
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        XCore.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    vehicleData.mods = XCore.Functions.GetVehicleProperties(vehicleData.ent)
    vehicleData.desc = data.desc

    TriggerServerEvent('x-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    XCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    XCore.Functions.Notify('Your car has been put up for sale! Price - $'..price, 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('x-occasion:client:refreshVehicles')
AddEventHandler('x-occasion:client:refreshVehicles', function()
    if inRange then
        XCore.Functions.TriggerCallback('x-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()

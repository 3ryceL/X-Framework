...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "sellVehicle") {
            ...
            XOccasions.setupSellContract(eventData)
        }

        if (eventData.action == "buyVehicle") {
            $('.buy-container').fadeIn(150);
            XOccasions.setupBuyContract(eventData, eventData.vehicleData)
        }
    });
});
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            ...
            $.post('https://x-vehiclesales/close');
            break;
    }
});
...
$(document).on('click', '#sell-vehicle', function(){
    if ($('.vehicle-sell-price').val() != "") {
        if (!isNaN($('.vehicle-sell-price').val())) {
            $.post('https://x-vehiclesales/sellVehicle', JSON.stringify({
            ...
            $.post('https://x-vehiclesales/error', JSON.stringify({
...
$(document).on('click', '#buy-vehicle', function(){
    $.post('https://x-vehiclesales/buyVehicle');
    $('.buy-container').fadeOut(100);
    $.post('https://x-vehiclesales/close');
});

XOccasions.setupSellContract = function(data) {
...
XOccasions.setupBuyContract = function(data, vData) {

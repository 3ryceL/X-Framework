            ...
            if PlayerJob.name == "trucker" then
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        TriggerServerEvent('x-trucker:server:DoBail', false)
                                    else
                                        XCore.Functions.Notify('This is not a commercial vehicle!', 'error')
                                    end
                                else
                                    XCore.Functions.Notify('You must be the driver to do this..')
                                end
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                TriggerServerEvent("x-trucker:server:01101110", JobsDone)
                                JobsDone = 0
				    ...
				    ClearAllBlipRoutes()
                                ...
                                XCore.Functions.Notify("You haven't done any work yet..", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, 38) then
                                            isWorking = true
                                            XCore.Functions.Progressbar("work_carrybox", "Take A Box Of Products", 2000, false, true, {
                            ...
                            elseif hasBox then
                                if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        ...
                                        XCore.Functions.Progressbar("work_dropbox", "Deliver Box Of Products", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                TriggerServerEvent("x-shops:server:RestockShopItems", CurrentLocation.store)
                                                XCore.Functions.Notify("You Have Delivered All Products, To The Next Point")
                                                local chance = math.random(1,100)
                                                if chance < 26 then
                                                    TriggerServerEvent('x-trucker:server:nano')
                                                end
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
						    ClearAllBlipRoutes()
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        XCore.Functions.Notify("You Went To All The Shops .. Time For Your Payslip!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	    ClearAllBlipRoutes()
            CurrentBlip = nil
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('x-trucker:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('x-trucker:client:SpawnVehicle')
AddEventHandler('x-trucker:client:SpawnVehicle', function()

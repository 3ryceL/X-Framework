            ...
            if InApartment then
                ...
                if CurrentDoorBell ~= 0 then
                    if entrancedist < 1.2 then
                        XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z + 0.1, '~g~G~w~ - Open door')
                ...
                if entrancedist < 3 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z, '~g~E~w~ - Leave Apartment')
                ...
                if stashdist < 1.2 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.stash.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.stash.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.stash.z, '~g~E~w~ - Stash')
                    if IsControlJustPressed(0, 38) then -- E
                ...
                elseif stashdist < 3 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.stash.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.stash.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.stash.z, 'Stash')
                end

                --Outfits
                if outfitsdist < 1.2 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.clothes.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.clothes.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.clothes.z, '~g~E~w~ - Outfits')
                    if IsControlJustPressed(0, 38) then -- E
                        TriggerEvent('x-clothing:client:openOutfitMenu')
                    end
                elseif outfitsdist < 3 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.clothes.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.clothes.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.clothes.z, 'Outfits')
                end

                --Logout
                if logoutdist < 1.5 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.logout.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.logout.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.logout.z, '~g~E~w~ - Log out')
                    if IsControlJustPressed(0, 38) then -- E
                        TriggerServerEvent('x-houses:server:LogoutLocation')
                    end
                elseif logoutdist < 3 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.logout.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.logout.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.logout.z, 'Log out')
                end
                ...
                if doorbelldist < 1.2 then
                    XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.doorbell.x, Apartments.Locations[ClosestHouse].coords.doorbell.y, Apartments.Locations[ClosestHouse].coords.doorbell.z, '~g~G~w~ - Ring Doorbell')
                    if IsControlJustPressed(0, 47) then -- G
                        MenuOwners()
                ...
                if IsOwned then
                   if entrance < 1.2 then
                        XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~E~w~ - Enter Apartment')
                        if IsControlJustPressed(0, 38) then -- E
                            XCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
                                if result ~= nil then
                ...
                elseif not IsOwned then
                    if entrance < 1.2 then
                        XCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~G~w~ - Change Apartment')
                        if IsControlJustPressed(0, 47) then -- G
...
AddEventHandler('apartments:client:setupSpawnUI', function(cData)
    XCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result ~= nil then
            TriggerEvent('x-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('x-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            TriggerEvent('x-spawn:client:setupSpawns', cData, true, Apartments.Locations)
            TriggerEvent('x-spawn:client:openUI', true)
        end
    end, cData.citizenid)
end)
...
RegisterNetEvent('x-apartments:client:LastLocationHouse')
AddEventHandler('x-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
AddEventHandler('apartments:client:RingDoor', function(player, house)
    ...
    XCore.Functions.Notify("Someone Is At The Door!")
end)
...
function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    XCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            XCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                data = exports['x-interior']:CreateApartmentFurnished(coords)
                Citizen.Wait(100)
                houseObj = data[1]
                POIOffsets = data[2]
                ...
                TriggerServerEvent('x-apartments:server:SetInsideMeta', house, apartmentId, true)
                

                TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
                TriggerServerEvent("XCore:NotifyServer:SetMetaData", "currentapartment", CurrentApartment)
            end, house)
        else
            if offset > 230 then
                offset = 210
        ...
        if new ~= nil then
            if new then
                TriggerEvent('x-interior:client:SetNewState', true)
            else
                TriggerEvent('x-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('x-interior:client:SetNewState', false)
        end
...
function LeaveApartment(house)
    ...
    exports['x-interior']:DespawnInterior(houseObj, function()
        ...
        TriggerServerEvent('x-apartments:server:SetInsideMeta', CurrentApartment, false)
        CurrentApartment = nil
        InApartment = false
        CurrentOffset = 0
        DoScreenFadeIn(1000)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
        TriggerServerEvent("XCore:NotifyServer:SetMetaData", "currentapartment", nil)
    end)
end
...
function SetClosestApartment()
    ...
    if current ~= ClosestHouse and isLoggedIn and not InApartment then
        ...
        XCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    XCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = PlayerPedId();
        MenuTitle = "Rang the door at: "
        ClearMenu()

        if apartments == nil then
            XCore.Functions.Notify("There is nobody home..", "error", 3500)
            closeMenuFull()
        else
...
function OutfitsLijst()
    XCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ped = PlayerPedId();
        MenuTitle = "My Outfits :"
        ClearMenu()

        if outfits == nil then
            XCore.Functions.Notify("You didnt save any outfits...", "error", 3500)
            closeMenuFull()
        else
...
function selectOutfit(oData)
    ...
    XCore.Functions.Notify(oData.outfitname.." chosen", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
...
function removeOutfit(oData)
    ...
    XCore.Functions.Notify(oData.outfitname.." has been deleted", "success", 2500)
    closeMenuFull()
end

...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('XCore:NotifyNotify', src, "You got a apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    ...
    TriggerClientEvent('XCore:NotifyNotify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		XCore.Functions.ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    XCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
XCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
XCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
XCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
XCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
XCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = XCore.Functions.GetPlayer(src)
    if Player ~= nil then
        exports.ghmattimysql:execute('SELECT * FROM apartments WHERE citizenid=@citizenid', {['@citizenid'] = Player.PlayerData.citizenid}, function(result)
...
XCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-apartments:server:SetInsideMeta')
AddEventHandler('x-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)

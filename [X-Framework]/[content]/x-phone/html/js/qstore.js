...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    X.Phone.Data.Applications[AppData.app] = null;

    $.post('https://x-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://x-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, X.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === X.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    X.Phone.Data.Applications[Installed.data.app] = Installed.data;

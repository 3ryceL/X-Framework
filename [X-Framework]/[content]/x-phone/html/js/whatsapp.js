...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    X.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://x-phone/ClearAlerts', JSON.stringify({
...
X.Phone.Functions.GetLastMessage = function(messages) {
...
X.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = X.Phone.Functions.GetLastMessage(chat.messages);
...
X.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://x-phone/SendMessage', JSON.stringify({
        ...
        X.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
// X.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('https://x-phone/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== X.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://x-phone/SharedLocation', JSON.stringify({

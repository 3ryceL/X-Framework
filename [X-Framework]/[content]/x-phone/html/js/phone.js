...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://x-phone/ClearRecentAlerts');
...
X.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    console.log(X.Phone.Data.AnonymousCall)

    $.post('https://x-phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: X.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== X.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (X.Phone.Data.AnonymousCall) {
                            X.Phone.Notifications.Add("fas fa-phone", "Phone", "You started a anonymous call!");
                        }
                        $(".phone-call-outgoing").css({"display":"block"});
                        $(".phone-call-incoming").css({"display":"none"});
                        $(".phone-call-ongoing").css({"display":"none"});
                        $(".phone-call-outgoing-caller").html(cData.name);
                        X.Phone.Functions.HeaderTextColor("white", 400);
                        X.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
                        setTimeout(function(){
                            $(".phone-app").css({"display":"none"});
                            X.Phone.Animations.TopSlideDown('.phone-application-container', 400, 0);
                            X.Phone.Functions.ToggleApp("phone-call", "block");
                        }, 450);
    
                        CallData.name = cData.name;
                        CallData.number = cData.number;
                    
                        X.Phone.Data.currentApplication = "phone-call";
                    } else {
                        X.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call!");
                    }
                } else {
                    X.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy!");
                }
            } else {
                X.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            X.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself!");
        }
    });
});
...
X.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== X.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            X.Phone.Functions.ToggleApp("phone", "none");
            X.Phone.Functions.ToggleApp("whatsapp", "block");
            X.Phone.Data.currentApplication = "whatsapp";
        
            $.post('https://x-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                X.Phone.Functions.SetupChatMessages(chat, {
        ...
        X.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    X.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://x-phone/EditContact', JSON.stringify({
            ...
            X.Phone.Functions.LoadContacts(PhoneContacts);
        });
        X.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        X.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://x-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    X.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://x-phone/AddNewContact', JSON.stringify({
        ...
        X.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://x-phone/RemoveSuggestion', JSON.stringify({
        ...
        X.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
        ...
        if (cData.number !== X.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    X.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call!");
                }
            } else {
                X.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            X.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call your own number!");
        }
    });
}

CancelOutgoingCall = function() {
    if (X.Phone.Data.currentApplication == "phone-call") {
        X.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!X.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                X.Phone.Functions.HeaderTextColor("white", 400);
                $("."+X.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
X.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    X.Phone.Functions.HeaderTextColor("white", 500);
    X.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    X.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    X.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://x-phone/AnswerCall');
});

X.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
X.Phone.Functions.SetupSuggestedContacts = function(Suggested) {

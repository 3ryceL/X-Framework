RegisterCommand('phone', function()
    if not PhoneData.isOpen then
        local IsHandcuffed = exports['x-policejob']:IsHandcuffed()
        if not IsHandcuffed then
            OpenPhone()
        else
            XCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNetEvent('x-phone:client:RaceNotify')
AddEventHandler('x-phone:client:RaceNotify', function(message)
...
RegisterNetEvent('x-phone:client:AddRecentCall')
AddEventHandler('x-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('x-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('x-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('x-phone:server:SaveMetaData', PhoneData.MetaData)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            XCore.Functions.TriggerCallback('x-phone:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    XCore.Functions.TriggerCallback('x-phone:server:GetPhoneData', function(pData)
        ...
        PhoneData.PlayerData = XCore.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    XCore.Functions.TriggerCallback('x-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            XCore.Functions.TriggerCallback('x-phone:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            XCore.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('x-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('x-phone:client:UpdateMails')
AddEventHandler('x-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('x-phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('x-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    XCore.Functions.TriggerCallback('x-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('x-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('x-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    XCore.Functions.TriggerCallback('x-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('x-phone:client:UpdateMessages')
AddEventHandler('x-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            XCore.Functions.TriggerCallback('x-phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('x-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("x-phone-new:client:BankNotify")
AddEventHandler("x-phone-new:client:BankNotify", function(text)
    --[[SendNUIMessage({
        action = "Notification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('x-phone:client:NewMailNotify')
AddEventHandler('x-phone:client:NewMailNotify', function(MailData)
    --if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
    ...
    TriggerServerEvent('x-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('x-phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('x-phone:client:UpdateAdverts')
AddEventHandler('x-phone:client:UpdateAdverts', function(Adverts, LastAd)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('x-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    XCore.Functions.TriggerCallback('x-phone:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    XCore.Functions.TriggerCallback('x-phone:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('x-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('x-phone:client:UpdateHashtags')
AddEventHandler('x-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('x-phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('x-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                end
            end
        end
    end
    ...
    TriggerServerEvent('x-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('x-phone:client:TransferMoney')
AddEventHandler('x-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    --if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Xank", text = "&#36;"..amount.." has been added to your account!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    --[[else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Xank", content = "&#36;"..amount.." has been added to your account!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end]]
end)


RegisterNetEvent('x-phone:client:UpdateTweets')
AddEventHandler('x-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('x-phone:client:GetMentioned')
AddEventHandler('x-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('x-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('x-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('x-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        XCore.Functions.TriggerCallback('x-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('x-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('x-phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('x-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('x-phone:server:SetCallState', false)
...
RegisterNetEvent('x-phone:client:CancelCall')
AddEventHandler('x-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('x-phone:client:GetCalled')
AddEventHandler('x-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('x-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('x-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('x-phone:client:AnswerCall')
AddEventHandler('x-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                XCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    XCore.Functions.TriggerCallback('x-phone:server:ScanPlate', function(result)
        XCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = XCore.Shared.Vehicles[vehname] ~= nil and XCore.Shared.Vehicles[vehname]["model"] or {["brand"] = "Unknown brand..", ["name"] = ""}
...
RegisterNetEvent('x-phone:client:addPoliceAlert')
AddEventHandler('x-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = XCore.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    XCore.Functions.Notify('GPS Location set: '..data.alert.title)
...
RegisterNetEvent('x-phone:client:GiveContactDetails')
AddEventHandler('x-phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('x-phone:server:GiveContactDetails', PlayerId)
    else
        XCore.Functions.Notify("No one nearby!", "error")
    end
end)

-- Citizen.CreateThread(function()
--     Wait(1000)
--     TriggerServerEvent('x-phone:server:GiveContactDetails', 1)
-- end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('x-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('x-phone:client:AddNewSuggestion')
AddEventHandler('x-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('x-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    XCore.Functions.TriggerCallback('x-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    XCore.Functions.TriggerCallback('x-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    XCore.Functions.TriggerCallback('x-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    XCore.Functions.TriggerCallback('x-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNetEvent('x-phone:client:RemoveBankMoney')
AddEventHandler('x-phone:client:RemoveBankMoney', function(amount)
    --if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('x-phone:client:AddTransaction')
AddEventHandler('x-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('x-phone:server:AddTransaction', Data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('x-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('x-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('x-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('x-phone:client:UpdateLapraces')
AddEventHandler('x-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('x-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['x-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['x-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('x-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:GetRacingData', function(RaceData)
        local ped = PlayerPedId()
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('x-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            XCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['x-lapraces']:IsInEditor())
    else
        cb(exports['x-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    XCore.Functions.TriggerCallback('x-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    XCore.Functions.Notify("GPS has been set to " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('x-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    XCore.Functions.TriggerCallback('x-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    XCore.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    XCore.Functions.TriggerCallback('x-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('x-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('x-phone:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('x-phone:RefreshPhone')
AddEventHandler('x-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = XCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['x-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)

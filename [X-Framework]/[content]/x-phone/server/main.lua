...
RegisterServerEvent('x-phone:server:AddAdvert')
AddEventHandler('x-phone:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('x-phone:client:UpdateAdverts', -1, Adverts, "@"..Player.PlayerData.charinfo.firstname..""..Player.PlayerData.charinfo.lastname)
end)

function GetOnlineStatus(number)
    local Target = XCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then retval = true end
    return retval
end

XCore.Functions.CreateCallback('x-phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
            ...
            exports.ghmattimysql:execute("SELECT * FROM phone_invoices WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(invoices)
                if invoices[1] ~= nil then
                    for k, v in pairs(invoices) do
                        local Ply = XCore.Functions.GetPlayerByCitizenId(v.sender)
                        if Ply ~= nil then
                            v.number = Ply.PlayerData.charinfo.phone
                        else
                            XCore.Functions.ExecuteSql(true, "SELECT * FROM `players` WHERE `citizenid` = '"..v.sender.."'", function(res)
                                if res[1] ~= nil then
                ...
                exports.ghmattimysql:execute("SELECT * FROM player_vehicles WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(garageresult)
                    if garageresult[1] ~= nil then
                        ...
                        --     if (XCore.Shared.Vehicles[v.vehicle] ~= nil) and (Garages[v.garage] ~= nil) then
                        --         v.garage = Garages[v.garage].label
                        --         v.vehicle = XCore.Shared.Vehicles[v.vehicle].name
                        --         v.brand = XCore.Shared.Vehicles[v.vehicle].brand
                        --     end
                        -- end
...
XCore.Functions.CreateCallback('x-phone:server:GetCallState', function(source, cb, ContactData)
    local Target = XCore.Functions.GetPlayerByPhone(ContactData.number)
...
RegisterServerEvent('x-phone:server:SetCallState')
AddEventHandler('x-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('x-phone:server:RemoveMail')
AddEventHandler('x-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        exports.ghmattimysql:execute('SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('x-phone:client:UpdateMails', src, mails)
        end)
    end)
end)
...
RegisterServerEvent('x-phone:server:sendNewMail')
AddEventHandler('x-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('x-phone:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('x-phone:server:sendNewMailToOffline')
AddEventHandler('x-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterServerEvent('x-phone:server:sendNewEventMail')
AddEventHandler('x-phone:server:sendNewEventMail', function(citizenid, mailData)
    if mailData.button == nil then
        exports.ghmattimysql:execute('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (@citizenid, @sender, @subject, @message, @mailid, @read)', {
            ['@citizenid'] = Player.PlayerData.citizenid,
            ['@sender'] = mailData.sender,
...
RegisterServerEvent('x-phone:server:ClearButtonData')
AddEventHandler('x-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-phone:server:MentionedPlayer')
AddEventHandler('x-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(XCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                XPhone.SetPhoneAlerts(Player.PlayerData.citizenid, "twitter")
                XPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                TriggerClientEvent('x-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
                exports.ghmattimysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..firstName.."%' AND `charinfo` LIKE '%"..lastName.."%'", function(result)
                    if result[1] ~= nil then
                        ...
                        XPhone.SetPhoneAlerts(MentionedTarget, "twitter")
                        XPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                    end
                end)
            end
        end
...
RegisterServerEvent('x-phone:server:CallContact')
AddEventHandler('x-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = XCore.Functions.GetPlayer(src)
    local Target = XCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('x-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

XCore.Functions.CreateCallback('x-phone:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = XCore.Functions.GetPlayer(src)
    local Trgt = XCore.Functions.GetPlayerByCitizenId(sender)
    local Invoices = {}
    ...
    if Trgt ~= nil then
        if Ply.PlayerData.money.bank >= amount then
            ...
            XCore.Functions.ExecuteSql(true, "DELETE FROM `phone_invoices` WHERE `invoiceid` = '"..invoiceId.."'")
            exports.ghmattimysql:execute("SELECT * FROM `phone_invoices` WHERE `citizenid` = '"..Ply.PlayerData.citizenid.."'", function(invoices)
                if invoices[1] ~= nil then
                    for k, v in pairs(invoices) do
                        local Target = XCore.Functions.GetPlayerByCitizenId(v.sender)
                        if Target ~= nil then
...
XCore.Functions.CreateCallback('x-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = XCore.Functions.GetPlayer(src)
    local Trgt = XCore.Functions.GetPlayerByCitizenId(sender)
    local Invoices = {}
...
RegisterServerEvent('x-phone:server:UpdateHashtags')
AddEventHandler('x-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('x-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

XPhone.AddMentionedTweet = function(citizenid, TweetData)
    if MentionedTweets[citizenid] == nil then MentionedTweets[citizenid] = {} end
    table.insert(MentionedTweets[citizenid], TweetData)
end

XPhone.SetPhoneAlerts = function(citizenid, app, alerts)
...
XCore.Functions.CreateCallback('x-phone:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        local Player = XCore.Functions.GetPlayerByPhone(v.number)
...
XCore.Functions.CreateCallback('x-phone:server:GetContactPicture', function(source, cb, Chat)
    local Player = XCore.Functions.GetPlayerByPhone(Chat.number)
...
XCore.Functions.CreateCallback('x-phone:server:GetPicture', function(source, cb, number)
    local Player = XCore.Functions.GetPlayerByPhone(number)
    local Picture = nil
...
RegisterServerEvent('x-phone:server:SetPhoneAlerts')
AddEventHandler('x-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = XCore.Functions.GetPlayer(src).citizenid
    XPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterServerEvent('x-phone:server:UpdateTweets')
AddEventHandler('x-phone:server:UpdateTweets', function(NewTweets, TweetData)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('x-phone:client:UpdateTweets', -1, src, Tweets, TwtData)
end)

RegisterServerEvent('x-phone:server:TransferMoney')
AddEventHandler('x-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = XCore.Functions.GetPlayer(src)

    exports.ghmattimysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            local reciever = XCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
            ...
            if reciever ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('x-phone:client:TransferMoney', reciever.PlayerData.source, amount, reciever.PlayerData.money.bank)
                end
            else
                local moneyInfo = json.decode(result[1].money)
            ...
            TriggerClientEvent('XCore:NotifyNotify', src, "This account number doesn't exist!", "error")
        end
    end)
end)

RegisterServerEvent('x-phone:server:EditContact')
AddEventHandler('x-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-phone:server:RemoveContact')
AddEventHandler('x-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-phone:server:AddNewContact')
AddEventHandler('x-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
RegisterServerEvent('x-phone:server:UpdateMessages')
AddEventHandler('x-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
    local SenderData = XCore.Functions.GetPlayer(src)

    exports.ghmattimysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..ChatNumber.."%'", function(Player)
        if Player[1] ~= nil then
            local TargetData = XCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
            ...
            if TargetData ~= nil then
                exports.ghmattimysql:execute("SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        TriggerClientEvent('x-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
                    else
                        -- Insert for target
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('x-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
                    end
                end)
            else
                exports.ghmattimysql:execute("SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
...
RegisterServerEvent('x-phone:server:AddRecentCall')
AddEventHandler('x-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = XCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('x-phone:client:AddRecentCall', src, data, label, type)

    local Trgt = XCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('x-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterServerEvent('x-phone:server:CancelCall')
AddEventHandler('x-phone:server:CancelCall', function(ContactData)
    local Ply = XCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('x-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('x-phone:server:AnswerCall')
AddEventHandler('x-phone:server:AnswerCall', function(CallData)
    local Ply = XCore.Functions.GetPlayerByPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('x-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('x-phone:server:SaveMetaData')
AddEventHandler('x-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
XCore.Functions.CreateCallback('x-phone:server:FetchResult', function(source, cb, search)
    local src = source
...
XCore.Functions.CreateCallback('x-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    exports.ghmattimysql:execute('SELECT * FROM `player_vehicles` WHERE `plate` LIKE "%'..search..'%" OR `citizenid` = "'..search..'"', function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                XCore.Functions.ExecuteSql(true, 'SELECT * FROM `players` WHERE `citizenid` = "'..result[k].citizenid..'"', function(player)
                    if player[1] ~= nil then 
                        local charinfo = json.decode(player[1].charinfo)
                        local vehicleInfo = XCore.Shared.Vehicles[result[k].vehicle]
...
XCore.Functions.CreateCallback('x-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
        exports.ghmattimysql:execute('SELECT * FROM `player_vehicles` WHERE `plate` = "'..plate..'"', function(result)
            if result[1] ~= nil then
                XCore.Functions.ExecuteSql(true, 'SELECT * FROM `players` WHERE `citizenid` = "'..result[1].citizenid..'"', function(player)
                    local charinfo = json.decode(player[1].charinfo)
                    vehicleData = {
                        plate = plate,
                        status = true,
        ...
        TriggerClientEvent('XCore:NotifyNotify', src, "No vehicle nearby..", "error")
        cb(nil)
    end
end)
...
function GenerateOwnerName()
    local names = {
        ...
        [15] = { name = "Antoon Henselijn", citizenid = "XC091H93" },
...
XCore.Functions.CreateCallback('x-phone:server:GetGarageVehicles', function(source, cb)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                local VehicleData = XCore.Shared.Vehicles[v.vehicle]
...
XCore.Functions.CreateCallback('x-phone:server:HasPhone', function(source, cb)
    local Player = XCore.Functions.GetPlayer(source)
...
XCore.Functions.CreateCallback('x-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = XCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        exports.ghmattimysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
            if result[1] ~= nil then
                ...
                TriggerClientEvent('XCore:NotifyNotify', src, "This account number does not exist!", "error")
                cb(false)
            end
        end)
    end
...
RegisterServerEvent('x-phone:server:GiveContactDetails')
AddEventHandler('x-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('x-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('x-phone:server:AddTransaction')
AddEventHandler('x-phone:server:AddTransaction', function(data)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
XCore.Functions.CreateCallback('x-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(XCore.Functions.GetPlayers()) do
        local Player = XCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterServerEvent('x-phone:server:InstallApplication')
AddEventHandler('x-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('x-phone:RefreshPhone', src)
end)

RegisterServerEvent('x-phone:server:RemoveInstallation')
AddEventHandler('x-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
XCore.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)

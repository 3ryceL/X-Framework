...
RegisterNetEvent("XCore:Client:OnPlayerLoaded")
AddEventHandler("XCore:Client:OnPlayerLoaded", function()
    isLoggedIn = true
end)

RegisterNetEvent("XCore:Client:OnPlayerUnload")
AddEventHandler("XCore:Client:OnPlayerUnload", function()
    isLoggedIn = false
end)
...
RegisterNetEvent("x-luckywheel:doRoll")
AddEventHandler("x-luckywheel:doRoll", function(_priceIndex) 
    _isRolling = true
    SetEntityHeading(_wheel, 328.0)
...
RegisterNetEvent("x-luckywheel:rollFinished")
AddEventHandler("x-luckywheel:rollFinished", function() 
    _isRolling = false
end)
...
function doRoll()
    if not _isRolling then
        ...
        TriggerServerEvent("x-luckywheel:getLucky")
...
Citizen.CreateThread(function()
	while true do
        ...
        if #(coords - vector3(_wheelPos.x, _wheelPos.y, _wheelPos.z)) < 1.5 and not _isRolling then
            XCore.Functions.DrawText3D( _wheelPos.x, _wheelPos.y, _wheelPos.z, 'Press ~g~[E] To Try Your Luck On The Wheel')

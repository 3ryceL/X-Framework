# x-gangs
Gangs for X-Core with Gang Support instead of Jobs, supports infinately scaling gangs. Super optimised down to 0.01ms with a gang, 0.00ms as a civilian.
...
# Installation
Add Gangs into x-core/shared.lua like this:
```lua
XShared.Gangs = json.decode(LoadResourceFile("x-gangs", "gangs.json"))

```
Add event to x-core/server/events.lua
```lua
RegisterServerEvent("XCore:NotifyServer:UpdateGangs")
AddEventHandler("XCore:NotifyServer:UpdateGangs", function(gangs)
	XShared.Gangs = gangs
	XCore.Shared.Gangs = gangs
end)
```
Add event to x-core/client/events.lua
```lua
RegisterNetEvent("XCore:NotifyServer:UpdateGangs")
AddEventHandler("XCore:NotifyServer:UpdateGangs", function(gangs)
	XShared.Gangs = gangs
	XCore.Shared.Gangs = gangs
end)
...
To enable lockable doors for gangs, you need to modify x-doorlocks/client/main.lua line 217 like this:
```lua

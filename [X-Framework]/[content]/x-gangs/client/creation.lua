...
RegisterNetEvent("x-gangs:client:BeginGangCreation")
AddEventHandler("x-gangs:client:BeginGangCreation", function(gangName, gangLabel)
    name = gangName
    label = gangLabel
    gang = {
        ["VehicleSpawner"] = {},
...
RegisterCommand("placestash", function()
    if isCreatingGang then
        if not hasStash then
            ...
            hasStash = true
            XCore.Functions.Notify("Gang Stash Placed", "success")
        else
            XCore.Functions.Notify("You have already placed the stash", "error")
        end
    else
        XCore.Functions.Notify("You are not creating a gang", "error")
    end
end)
...
RegisterCommand("placegarage", function()
    if isCreatingGang then
        if not hasGarage then
            ...
            XCore.Functions.Notify("You have already placed the garage", "error")
        end
    else
        XCore.Functions.Notify("You are not creating a gang", "error")
    end
...
RegisterCommand("finishgang", function()
    if isCreatingGang then
        if hasGarage and hasStash then
            TriggerServerEvent("x-gangs:server:creategang", gang, name, label)
            gang = nil
            hasGarage, hasStash = false, false
        else
            XCore.Functions.Notify("You are not finished", "error")
        end
    else
        XCore.Functions.Notify("You are not creating a gang", "error")
    end
...
RegisterNUICallback("SetVehicleColour", function(data, cb)
    ...
    for k, v in pairs(vehicleList) do
        if XCore.Shared.Vehicles[v] ~= nil then
            local name = XCore.Shared.Vehicles[v].name
            gang["VehicleSpawner"]["vehicles"][v] = name
        else
            XCore.Functions.Notify(v.." does not exist in x-core/shared-modules/vehicles.lua", "error")
        end
    end

    hasGarage = true
    XCore.Functions.Notify("Garage Placed", "success")

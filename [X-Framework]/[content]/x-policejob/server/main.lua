...
AddEventHandler('police:server:CheckBills', function()
    ...
    exports.ghmattimysql:execute('SELECT * FROM bills WHERE citizenid=@citizenid AND type=@type', {['@citizenid'] = Player.PlayerData.citizenid, ['@type'] = 'police'}, function(result)
        if result[1] ~= nil then
            ...
            TriggerEvent('x-moneysafe:server:DepositMoney', "police", totalAmount, "bills")
		end
	end)
end)
...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('XCore:NotifyNotify', src, "Vehicle is taken out of Impound!")  
end)
...
AddEventHandler('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    local CuffedPlayer = XCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = XCore.Functions.GetPlayer(source)
    local EscortPlayer = XCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('XCore:NotifyNotify', OtherPlayer.PlayerData.source, "You received a fine of $"..price)
        end
    end
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('XCore:NotifyNotify', src, "You sent the person to prison for "..time.." months")
        end
    end
end)
...
AddEventHandler('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = XCore.Functions.GetPlayer(src)
    local alertData = {
...
AddEventHandler('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = XCore.Functions.GetPlayer(playerId)
    if SearchedPlayer ~= nil then 
        TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "Person has $"..SearchedPlayer.PlayerData.money["cash"]..",- on him..")
        TriggerClientEvent('XCore:NotifyNotify', SearchedPlayer.PlayerData.source, "You are being searched..")
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items["moneybag"], "add")
        TriggerClientEvent('XCore:NotifyNotify', SearchedPlayer.PlayerData.source, "Your cash is confiscated..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('XCore:NotifyNotify', SearchedPlayer.PlayerData.source, "Your driving license has been confiscated..")
        else
            TriggerClientEvent('XCore:NotifyNotify', src, "Can't confiscate driving license..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('XCore:NotifyNotify', SearchedPlayer.PlayerData.source, "You have been robbed of $"..money)
	TriggerClientEvent('XCore:NotifyNotify', Player.PlayerData.source, "You have stolen $"..money)
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('XCore:NotifyNotify', src, "Vehicle taken into depot for $"..price.."!")
        else
            exports['ghmattimysql']:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 2, ['@plate'] = plate})
            TriggerClientEvent('XCore:NotifyNotify', src, "Vehicle completely seized!")
        end
    end
end)
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, bloodInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, XCore.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveBlooddrop", -1, bloodId)
            BloodDrops[bloodId] = nil
        end
    else
        TriggerClientEvent('XCore:NotifyNotify', src, "You must have an empty evidence bag with you", "error")
    end
end)
...
AddEventHandler('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = XCore.Shared.Weapons[weapon]
...
AddEventHandler('police:server:showFingerprint', function(playerId)
    local src = source
    local Player = XCore.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    local Target = XCore.Functions.GetPlayer(targetId)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]

    if TrackerMeta then
        Target.Functions.SetMetaData("tracker", false)
        TriggerClientEvent('XCore:NotifyNotify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('XCore:NotifyNotify', source, 'You took off an ankle bracelet from '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('XCore:NotifyNotify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('XCore:NotifyNotify', source, 'You put on an ankle strap to '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = XCore.Functions.GetPlayer(source)
    local TrackerMeta = Target.PlayerData.metadata["tracker"]
    ...
    TriggerClientEvent("x-phone:client:addPoliceAlert", requestId, alertData)
end)
...
XCore.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = XCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

XCore.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = XCore.Functions.GetPlayer(playerId)
    local statList = {}
	if Player ~= nil then
...
XCore.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = XCore.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(XCore.Shared.Weapons[weapon]["name"])
    local retval = false
...
XCore.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    for k, v in pairs(XCore.Functions.GetPlayers()) do
        local Player = XCore.Functions.GetPlayer(v)
        if Player ~= nil then 
...
XCore.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
XCore.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
XCore.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
...
--[[ XCore.Commands.Add("setpolice", "Hire An Officer (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
...
XCore.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
--[[ XCore.Commands.Add("firepolice", "Fire An Officer (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(tonumber(args[1]))
    local Myself = XCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Myself.PlayerData.job.name == "police" and Myself.PlayerData.job.onduty) and IsHighCommand(Myself.PlayerData.citizenid) then
...
XCore.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name="type", help="Type object you want or 'delete' to delete"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('XCore:NotifyNotify', source, 'For Emergency Services Only', 'error')
    end
end)

XCore.Commands.Add("cuff", "Cuff Player (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("palert", "Make a police alert", {{name="alert", help="The Police alert"}}, false, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerEvent("x-log:server:CreateLog", "palert", "Police alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..msg, false)
...
XCore.Commands.Add("escort", "Escort Player", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    TriggerClientEvent("police:client:EscortPlayer", source)
end)

--XCore.Commands.Add("mdt", "Open MDT (Police Only)", {}, false, function(source, args)
--	local Player = XCore.Functions.GetPlayer(source)
--    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("callsign", "Give Yourself A Callsign", {{name="name", help="Name of your callsign"}}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

XCore.Commands.Add("clearcasings", "Clear Area of Casings (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("jail", "Jail Player (Police Only)", {{name="id", help="Player ID"},{name="time", help="Time they have to be in jail"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("unjail", "Unjail Player (Police Only)", {{name="id", help="Player ID"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
XCore.Commands.Add("clearblood", "Clear The Area of Blood (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("seizecash", "Seize Cash (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
XCore.Commands.Add("sc", "Soft Cuff (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("cam", "View Security Camera (Police Only)", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("flagplate", "Flag A Plate (Police Only)", {{name="plate", help="License"}, {name="reason", help="Reason of flagging the vehicle"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('XCore:NotifyNotify', source, "Vehicle ("..args[1]:upper()..") is flagged for: "..table.concat(reason, " "))
    else
        TriggerClientEvent('XCore:NotifyNotify', source, 'For Emergency Services Only', 'error')
    end
end)

XCore.Commands.Add("unflagplate", "Unflag A Plate (Police Only)", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                Plates[args[1]:upper()].isflagged = false
                TriggerClientEvent('XCore:NotifyNotify', source, "Vehicle ("..args[1]:upper()..") is unflagged")
            else
...
XCore.Commands.Add("plateinfo", "Run A Plate (Police Only)", {{name="plate", help="License plate"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
...
XCore.Commands.Add("depot", "Impound With Price (Police Only)", {{name="prijs", help="Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("impound", "Impound A Vehicle (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Commands.Add("paytow", "Pay Tow Driver (Police, EMS Only)", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('XCore:NotifyNotify', source, 'You paid a bergnet worker')
            else
                TriggerClientEvent('XCore:NotifyNotify', source, 'Person is not a bergnet worker', "error")
            end
        end
    else
        TriggerClientEvent('XCore:NotifyNotify', source, 'For Emergency Services Only', 'error')
...
XCore.Commands.Add("paylawyer", "Pay Lawyer (Police, Judge Only)", {{name="id", help="ID of the player"}}, true, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('XCore:NotifyNotify', source, 'You paid a lawyer')
            else
                TriggerClientEvent('XCore:NotifyNotify', source, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('XCore:NotifyNotify', source, 'For Emergency Services Only', 'error')
...
XCore.Commands.Add("radar", "Enable Police Radar (Police Only)", {}, false, function(source, args)
	local Player = XCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
XCore.Functions.CreateUseableItem("handcuffs", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)
...
XCore.Commands.Add("911", "Send a report to emergency services", {{name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerEvent("x-log:server:CreateLog", "911", "911 alert", "blue", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Alert:** " ..message, false)
    else
        TriggerClientEvent('XCore:NotifyNotify', source, 'You dont have a phone', 'error')
    end
end)

XCore.Commands.Add("911a", "Send an anonymous report to emergency services (gives no location)", {{name="message", help="Message you want to send"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = XCore.Functions.GetPlayer(source)
...
XCore.Commands.Add("911r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="bericht", help="Message you want to send"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    local OtherPlayer = XCore.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
    local Prefix = "POLICE"
...
XCore.Commands.Add("anklet", "Attach Tracking Anklet (Police Only)", {}, false, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
...
XCore.Commands.Add("ankletlocation", "Get the location of a persons anklet", {{"csn", "CSN of the person"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    
    if Player.PlayerData.job.name == "police" then
        if args[1] ~= nil then
            local citizenid = args[1]
            local Target = XCore.Functions.GetPlayerByCitizenId(citizenid)
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('XCore:NotifyNotify', source, 'This person doesn\'t have an anklet on.', 'error')
                end
            end
        end
    else
...
XCore.Commands.Add("removeanklet", "Remove Tracking Anklet (Police Only)", {{"bsn", "BSN of person"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
        ...
        if args[1] ~= nil then
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('XCore:NotifyNotify', source, 'This person does not have an anklet', 'error')
                end
            end
        end
    else
...
XCore.Commands.Add("ebutton", "Respond To A Call (Police, EMS, Mechanic Only)", {}, false, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
...
XCore.Commands.Add("takedrivinglicense", "Seize Drivers License (Police Only)", {}, false, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
XCore.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed) (Police Only)", {{"id", "ID of the person"}}, true, function(source, args)
    local Player = XCore.Functions.GetPlayer(source)
    local OtherPlayer = XCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", source, XCore.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('XCore:NotifyNotify', source, "You must have an empty evidence bag with you", "error")
        end
    end
end)

XCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)
    if Player.Functions.GetItemBySlot(item.slot) ~= nil then
...
XCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    for k, v in pairs(XCore.Functions.GetPlayers()) do
        local Player = XCore.Functions.GetPlayer(v)
        if Player ~= nil then 

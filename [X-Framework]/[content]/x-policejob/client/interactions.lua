Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not XCore.Functions.GetPlayerData().metadata["isdead"] then
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            XCore.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            XCore.Functions.Notify("Time must be higher than 0..", "error")
        end
    else
        XCore.Functions.Notify("No one nearby!", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                XCore.Functions.Notify("You cant cuff someone in a vehicle", "error")
            end
        else
            XCore.Functions.Notify("No one nearby!", "error")
        end
            ...
            XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(result)
                if result then 
                    ...
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                        ...
                        XCore.Functions.Notify("You can\'t cuff someone in a vehicle", "error")
                    end
                else
                    XCore.Functions.Notify("You don\'t have handcuffs on you", "error")
                end
            end, "handcuffs")
        else
            XCore.Functions.Notify("No one nearby!", "error")
...
AddEventHandler('police:client:GetKidnappedDragger', function(playerId)
    XCore.Functions.GetPlayerData(function(PlayerData)
        ...
        TriggerEvent('x-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            XCore.Functions.Notify("You are cuffed!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            XCore.Functions.Notify("You are cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(PlayerPedId())
        XCore.Functions.Notify("You are uncuffed!")
    end
end)
...
function IsTargetDead(playerId)
    local retval = false
    XCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval

...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	XCore.Functions.Progressbar("clear_blooddrops", "Clearing blood..", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			XCore.Functions.Notify("Blood cleared :)")
		end
    end, function() -- Cancel
        XCore.Functions.Notify("Blood not cleared..", "error")
    end)
end)
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(PlayerPedId())
	local casingList = {}
	XCore.Functions.Progressbar("clear_casings", "Removing bullet sleeves..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			XCore.Functions.Notify("Bullet sleeves removed :)")
		end
    end, function() -- Cancel
        XCore.Functions.Notify("Bullet sleeves not removed", "error")
    end)
end)
			...
			if #(pos - vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
				...
				if IsControlJustReleased(0, 47) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[XCore.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
		...
		if isArmed then
			if PlayerJob.name ~= "police" then
				if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					...
					if XCore.Shared.Weapons[currentWeapon]["ammotype"] ~= "AMMO_PISTOL" then
						automatic = true
					end
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		XCore.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)

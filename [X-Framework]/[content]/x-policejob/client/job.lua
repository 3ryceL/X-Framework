                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 5 then
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                TriggerEvent('x-policealerts:ToggleDuty', onDuty)
                                if onDuty then
                            ...
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                                        ...
                                        XCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                    ...
                    if #(pos - vector3(v.x, v.y, v.z)) < 4.5 then
                        if onDuty then
                            if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..XCore.Functions.GetPlayerData().citizenid)
                                    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..XCore.Functions.GetPlayerData().citizenid)
                                end
                            elseif #(pos - vector3(v.x, v.y, v.z)) < 2.5 then
    ...
    XCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                XCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result ~= nil then
...
function ImpoundVehicleList()
    XCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Impounded Vehicles:"
        ClearMenu()

        if result == nil then
            XCore.Functions.Notify("There are no impounded vehicles", "error", 5000)
            closeMenuFull()
        else
            for k, v in pairs(result) do
                ...
                Menu.addButton(XCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutImpound", v, "Impounded", " Engine: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
...
function OutfitsLijst()
    XCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            XCore.Functions.Notify("You have no outfits saved...", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    XCore.Functions.Notify(oData.outfitname.." is deleted", "success", 2500)
    closeMenuFull()
end
...
--[[     local citizenid = XCore.Functions.GetPlayerData().citizenid

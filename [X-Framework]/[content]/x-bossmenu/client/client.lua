...
CreateThread(function()
    while true do
        ...
        if isLoggedIn and PlayerJob.name ~= nil then
            ...
            for k, v in pairs(XCore.Shared.Jobs[PlayerJob.name]) do
                v = XCore.Shared.Jobs[PlayerJob.name]["bossmenu"]
                if v ~= nil then
                    if #(pos - v) < 2 then
                        if #(pos - v) < 1.0 then
                            DrawText3D(v, "~g~E~w~ - Boss Menu")
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("x-bossmenu:server:openMenu")
                            end
...
RegisterNetEvent('x-bossmenu:client:openMenu')
AddEventHandler('x-bossmenu:client:openMenu', function(employees, jobdata)
...
RegisterNetEvent('x-bossmenu:client:refreshPage')
AddEventHandler('x-bossmenu:client:refreshPage', function(data, list)
...
RegisterNetEvent('x-bossmenu:client:refreshSociety')
AddEventHandler('x-bossmenu:client:refreshSociety', function(job, data)
    if XCore and XCore.Functions.GetPlayerData().job.name == job then
        SendNUIMessage({
            open = true,
...
RegisterNUICallback('openStash', function(data)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "boss_" .. XCore.Functions.GetPlayerData().job.name, {
        maxweight = 4000000,
        slots = 500,
    })

    TriggerEvent("inventory:client:SetCurrentStash", "boss_" .. XCore.Functions.GetPlayerData().job.name)
end)
...
RegisterNUICallback('giveJob', function(data)
    TriggerServerEvent('x-bossmenu:server:giveJob', data)
end)
...
RegisterNUICallback('openRecruit', function(data)
    CreateThread(function()
        ...
        for k,v in pairs(XCore.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
            if v and v ~= PlayerId() then
                table.insert(players, GetPlayerServerId(v))
            end
        end

        TriggerServerEvent("x-bossmenu:server:updateNearbys", players)
    end)
end)

RegisterNUICallback('changeGrade', function(data)
    TriggerServerEvent('x-bossmenu:server:updateGrade', data)
end)

RegisterNUICallback('fireEmployee', function(data)
    TriggerServerEvent('x-bossmenu:server:fireEmployee', data)
end)
...
RegisterNUICallback('withdraw', function(data)
    ...
    TriggerServerEvent("x-bossmenu:server:withdrawMoney", amount)
end)

RegisterNUICallback('deposit', function(data)
    local amount = tonumber(data.amount)
    TriggerServerEvent("x-bossmenu:server:depositMoney", amount)
end)

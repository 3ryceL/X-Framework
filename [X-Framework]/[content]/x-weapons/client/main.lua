...
Citizen.CreateThread(function()
    ...
    if XCore.Functions.GetPlayerData() ~= nil then
        ...
        XCore.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
...
Citizen.CreateThread(function()
    while true do
        if isLoggedIn then
            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        if XCore.Shared.Weapons[weapon]["name"] == "weapon_snowball" then
                            TriggerServerEvent('XCore:NotifyServer:RemoveItem', "snowball", 1)
                        else
                            if ammo > 0 then
			...
			local weapon = GetSelectedPedWeapon(ped)
                        TriggerEvent('inventory:client:CheckWeapon', XCore.Shared.Weapons[weapon]["name"])
                        XCore.Functions.Notify("This weapon is broken and can not be used..", "error")
                        MultiplierAmount = 0
                    end
                end
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            if ammo - 1 < 1 then
                SetAmmoInClip(ped, GetHashKey(XCore.Shared.Weapons[weapon]["name"]), 1)
            end
        end
...
AddEventHandler('weapon:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData ~= nil then
        if XCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and XCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                XCore.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(4000, 6000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if XCore.Shared.Weapons[weapon] ~= nil then
                        AddAmmoToPed(ped,weapon,retval)
                        TaskReloadWeapon(ped)
                        TriggerServerEvent("weapons:server:AddWeaponAmmo", CurrentWeaponData, total + retval)
                        TriggerServerEvent('XCore:NotifyServer:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items[itemData.name], "remove")
                        TriggerEvent('XCore:NotifyNotify', 'Reloaded', "success")
                    end
                end, function()
                    XCore.Functions.Notify("Canceled", "error")
                end)
            else
                XCore.Functions.Notify("Max Ammo Capacity", "error")
            end
        else
            XCore.Functions.Notify("You Have No Weapon", "error")
        end
    else
        XCore.Functions.Notify("You Have No Weapon.", "error")
    end
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                                if not data.RepairingData.Ready then
                                    local WeaponData = XCore.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        XCore.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
...
AddEventHandler("weapons:client:EquipAttachment", function(ItemData, attachment)
    local ped = PlayerPedId()
    local weapon = GetSelectedPedWeapon(ped)
    local WeaponData = XCore.Shared.Weapons[weapon]
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if Config.WeaponAttachments[WeaponData.name] ~= nil then
            if Config.WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                XCore.Functions.Notify("This weapon does not support this attachment..", "error")
            end
        end
    else
        XCore.Functions.Notify("You dont have a weapon in ur hand..", "error")
    end
end)

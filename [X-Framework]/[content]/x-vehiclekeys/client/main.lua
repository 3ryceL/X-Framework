...
Citizen.CreateThread(function()
    while true do
	    ...
	    local vehicle = GetVehiclePedIsIn(ped)
            if IsPedInAnyVehicle(ped, false) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped, true), -1) == ped then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(ped, false) then
                    XCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(ped, false) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped, false), -1) == ped and XCore ~= nil and not IsHotwiring then
            local veh = GetVehiclePedIsIn(ped, false)
            SetVehicleEngineOn(veh, false, false, true)
            --[[local veh = GetVehiclePedIsIn(PlayerPedId(), false)
            local vehpos = GetOffsetFromEntityInWorldCoords(veh, 0, 1.5, 0.5)
            XCore.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire")
            SetVehicleEngineOn(veh, false, false, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and XCore ~= nil then
            local ped = PlayerPedId()
            if GetVehiclePedIsTryingToEnter(ped) ~= nil and GetVehiclePedIsTryingToEnter(ped) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        XCore.Functions.Progressbar("rob_keys", "Grabbing keys..", 3000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            --[[if XCore.Functions.GetPlayerData().job.name ~= "police" then
...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
    --XCore.Functions.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
function LockVehicle()
    local ped = PlayerPedId()
    local veh = XCore.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and #(pos - vehpos) < 7.5 then
        XCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                            XCore.Functions.Notify("Vehicle locked!")
                        else
                            XCore.Functions.Notify("Something went wrong with the locking system!")
                        end
                    else
                        Citizen.Wait(750)
                        ClearPedTasks(ped)
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 1)then
                            XCore.Functions.Notify("Vehicle unlocked!")
                        else
                            XCore.Functions.Notify("Something went wrong with the locking system!")
                        end
                    end
                ...
                XCore.Functions.Notify('You don\'t have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if #(pos - vehpos) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                XCore.Functions.Progressbar("lockpick_vehicledoor", "Breaking the door open..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        XCore.Functions.Notify("Door open!")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        if SucceededAttempts + 1 >= NeededAttempts then
                            ...
                            XCore.Functions.Notify("Lockpicking succeeded!")
                            HasKey = true
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            IsHotwiring = false
                            FailedAttemps = 0
                            ...
                            if vehicle ~= nil and vehicle ~= 0 then
                                ...
                                HasKey = false
                                SetVehicleEngineOn(vehicle, false, false, true)
                                XCore.Functions.Notify("You have to be in the vehicle", "error")
                                IsHotwiring = false
			...
			if randChance == 3 then
				if isAdvanced then
				    ...
				    TriggerServerEvent("XCore:NotifyServer:RemoveItem", "lockpick", 1)
                        ...
                        XCore.Functions.Notify("Lockpicking failed!", "error")
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function Hotwire()
    if not HasKey then
        ...
        PoliceCall()
        XCore.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if (math.random(0, 100) < 10) then
                HasKey = true
                XCore.Functions.Notify("Hotwire succeeded!")
            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                XCore.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    if XCore.Shared.Vehicles[modelName] ~= nil then
                        Name = XCore.Shared.Vehicles[modelName]["brand"] .. ' ' .. XCore.Shared.Vehicles[modelName]["name"]
                    else
                        Name = "Unknown"
                    end

...
AddEventHandler("XCore:NotifyClient:OnPlayerLoaded", function()
    ...
    XCore.Functions.TriggerCallback('x-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
Citizen.CreateThread(function()
    Wait(500)
    if XCore.Functions.GetPlayerData() ~= nil then
        PlayerJob = XCore.Functions.GetPlayerData().job
        onDuty = true
    end
end)

RegisterNetEvent('XCore:NotifyClient:SetDuty')
AddEventHandler('XCore:NotifyClient:SetDuty', function(duty)
    onDuty = duty
end)
...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = XCore.Shared.Items["electronickit"]["name"], image = XCore.Shared.Items["electronickit"]["image"]},
        [2] = {name = XCore.Shared.Items["trojan_usb"]["name"], image = XCore.Shared.Items["trojan_usb"]["image"]},
    }
    local requiredItems2 = {
        [1] = {name = XCore.Shared.Items["thermite"]["name"], image = XCore.Shared.Items["thermite"]["image"]},
    }
    while true do
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                XCore.Functions.Notify('Minimum Of '..Config.MinimumFleecaPolice..' Police Needed', "error")
                                            end
                                        end
                                    end
                                end
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        XCore.Functions.TriggerCallback('x-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = #(pos - vector3(Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"]))
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        XCore.Functions.Progressbar("hack_gate", "Connecting the hacking device ..", math.random(5000, 10000), false, true, {
                                            ...
                                            TriggerServerEvent("XCore:NotifyServer:RemoveItem", "electronickit", 1)
                                            TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items["electronickit"], "remove")
                                            TriggerServerEvent("XCore:NotifyServer:RemoveItem", "trojan_usb", 1)
                                            TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items["trojan_usb"], "remove")
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("x-bankrobbery:server:callCops", "small", closestBank, streetLabel, pos)
                                                    copsCalled = true
                                                end
                                            end
                                        end, function() -- Cancel
                                            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                                            XCore.Functions.Notify("Canceled..", "error")
                                        end)
                                    else
                                        XCore.Functions.Notify("You're missing an item ..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                XCore.Functions.Notify("Looks like the bank is already open ..", "error")
                            end
                        else
                            XCore.Functions.Notify('Minimum Of '..Config.MinimumFleecaPolice..' Police Needed', "error")
                        end
                ...
                XCore.Functions.Notify("The security lock is active, opening the door is currently not possible.", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('x-bankrobbery:client:setBankState')
AddEventHandler('x-bankrobbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('x-bankrobbery:client:enableAllBankSecurity')
AddEventHandler('x-bankrobbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('x-bankrobbery:client:disableAllBankSecurity')
AddEventHandler('x-bankrobbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('x-bankrobbery:client:BankSecurity')
AddEventHandler('x-bankrobbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                XCore.Functions.Progressbar("open_locker_drill", "Breaking open the safe ..", math.random(18000, 30000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('x-bankrobbery:server:recieveItem', 'paleto')
                    XCore.Functions.Notify("Successful!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                XCore.Functions.Notify("Looks like the safe lock is too strong ..", "error")
                TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(hasItem)
            if hasItem then
                ...
                XCore.Functions.Progressbar("open_locker_drill", "Breaking open the safe ..", math.random(18000, 30000), false, true, {
                    ...
                    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('x-bankrobbery:server:recieveItem', 'pacific')
                    XCore.Functions.Notify("Successful!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
        ...
        IsDrilling = true
        XCore.Functions.Progressbar("open_locker", "Breaking open the safe ..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('x-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('x-bankrobbery:server:recieveItem', 'small')
            XCore.Functions.Notify("Successful!", "success")
            IsDrilling = false
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
...
RegisterNetEvent('x-bankrobbery:client:setLockerState')
AddEventHandler('x-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('x-bankrobbery:client:ResetFleecaLockers')
AddEventHandler('x-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('x-bankrobbery:client:robberyCall')
AddEventHandler('x-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        if type == "small" then
            ...
            TriggerEvent('x-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                ...
                callSign = XCore.Functions.GetPlayerData().metadata["callsign"],
            })
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('x-bankrobbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end

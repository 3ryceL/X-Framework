        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    XCore.Functions.Notify("It seems that the fuses have blown.", "error")
                end
            else
                XCore.Functions.Notify('Minimum Of '..Config.MinimumThermitePolice..' Police Needed', "error")
            end
        end
...
RegisterNetEvent('x-bankrobbery:client:SetStationStatus')
AddEventHandler('x-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        XCore.Functions.Notify("Thermite is going off in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        ...
        TriggerServerEvent("x-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        XCore.Functions.Notify("The door is open", "success")
        TriggerServerEvent('x-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)

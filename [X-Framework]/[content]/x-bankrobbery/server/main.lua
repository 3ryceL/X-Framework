...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            TriggerEvent("x-weathersync:server:toggleBlackout")
            TriggerClientEvent("police:client:EnableAllCameras", -1)
            TriggerClientEvent("x-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('x-bankrobbery:server:setBankState')
AddEventHandler('x-bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('x-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('x-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('x-bankrobbery:server:setTimeout')
        end
    elseif bankId == "pacific" then
        if not robberyBusy then
            Config.BigBanks["pacific"]["isOpened"] = state
            TriggerClientEvent('x-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('x-scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('x-bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
            ...
            TriggerClientEvent('x-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('x-banking:server:SetBankClosed', bankId, true)
            TriggerEvent('x-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('x-bankrobbery:server:SetSmallbankTimeout', bankId)
        end
    end
    robberyBusy = true
end)

RegisterServerEvent('x-bankrobbery:server:setLockerState')
AddEventHandler('x-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
    ...
    TriggerClientEvent('x-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterServerEvent('x-bankrobbery:server:recieveItem')
AddEventHandler('x-bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 if Config.RewardTypes[itemType].type == "item" then
                     ...
                     TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items[item.item], "add")
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['weapon_vintagepistol'], "add")
        end
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['markedbills'], "add")
                 ...
                 TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, XCore.Shared.Items['weapon_minismg'], "add")
            end
...
XCore.Functions.CreateCallback('x-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

XCore.Functions.CreateCallback('x-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('x-bankrobbery:server:setTimeout')
AddEventHandler('x-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('x-scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('x-scoreboard:server:SetActivityBusy', "pacific", false)
                ...
                TriggerClientEvent('x-bankrobbery:client:ClearTimeoutDoors', -1)
...
RegisterServerEvent('x-bankrobbery:server:SetSmallbankTimeout')
AddEventHandler('x-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
            	TriggerClientEvent('x-bankrobbery:client:ResetFleecaLockers', -1, BankId)
            	TriggerEvent('x-banking:server:SetBankClosed', BankId, false)
            end)
	end
    end
end)

RegisterServerEvent('x-bankrobbery:server:callCops')
AddEventHandler('x-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    ...
    TriggerClientEvent("x-bankrobbery:client:robberyCall", -1, type, bank, streetLabel, coords)
    TriggerClientEvent("x-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterServerEvent('x-bankrobbery:server:SetStationStatus')
AddEventHandler('x-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("x-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("x-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("x-bankrobbery:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("x-bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("x-bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("x-bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("x-bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("x-bankrobbery:client:BankSecurity", 5, false)
    end
end
...
XCore.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('XCore:NotifyNotify', source, "You're missing ignition source ", "error")
    end
end)

XCore.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_01') ~= nil then
        TriggerClientEvent("x-bankrobbery:UseBankcardA", source)
    end
end)

XCore.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_02') ~= nil then
        TriggerClientEvent("x-bankrobbery:UseBankcardB", source)
    end
end)

XCore.Functions.CreateUseableItem("electronickit", function(source, item)
    local Player = XCore.Functions.GetPlayer(source)

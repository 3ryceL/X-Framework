...
XCore.Commands.Add('cash', 'Check Cash Balance', {}, false, function(source, args)
	TriggerClientEvent('hud:client:ShowMoney', source, 'cash')
end)

XCore.Commands.Add('bank', 'Check Bank Balance', {}, false, function(source, args)
...
AddEventHandler('hud:server:GainStress', function(amount)
    ...
    if Player ~= nil and Player.PlayerData.job.name ~= 'police' then
        ...
        TriggerClientEvent('XCore:NotifyNotify', src, 'Getting Stressed', 'error', 1500)
	end
end)
...
AddEventHandler('hud:server:RelieveStress', function(amount)
    ...
    if Player ~= nil then
        ...
        Player.Functions.SetMetaData('stress', newStress)
        TriggerClientEvent('hud:client:UpdateStress', src, newStress)
        TriggerClientEvent('XCore:NotifyNotify', src, 'You Are Relaxing')
	end
end)
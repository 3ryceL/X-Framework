...
RegisterNetEvent('hud:client:UpdateNeeds') -- Triggered in x-core
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    while true do 
        if XCore ~= nil and isLoggedIn and XHud.Show then
...
Citizen.CreateThread(function() 
    while true do
        Citizen.Wait(1000)
        if IsPedInAnyVehicle(PlayerPedId()) and isLoggedIn and XHud.Show then
            DisplayRadar(true)
            SendNUIMessage({
                action = 'car',
                show = true,
...
AddEventHandler('hud:client:SetMoney', function()
    ...
    if XHud.Money.ShowConstant then
        SendNUIMessage({
            action = 'open',
            cash = cashAmount,
            bank = bankAmount,
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif stress >= XHud.MinimumStress then
            local ShakeIntensity = GetShakeIntensity(stress)
            ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', ShakeIntensity)
...
function GetShakeIntensity(stresslevel)
    local retval = 0.05
    for k, v in pairs(XHud.Intensity['shake']) do
...
function GetEffectInterval(stresslevel)
    local retval = 60000
    for k, v in pairs(XHud.EffectInterval) do
        if stresslevel >= v.min and stresslevel < v.max then
            retval = v.timeout
            break
        end

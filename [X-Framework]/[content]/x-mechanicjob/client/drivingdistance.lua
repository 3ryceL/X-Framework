                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            XCore.Functions.TriggerCallback('x-vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
                ...
                if vehiclemeters ~= -1 then
                    if seat == ped then
                        if previousvehiclepos ~= nil then
                            ...
                            if DamageKey ~= nil then
                                ...
                                if chance == odd then
                                    for k, v in pairs(Config.Damages) do
                                        ...
                                        TriggerServerEvent('x-vehicletuning:server:SetPartLevel', plate, k, newDamage)
                                    end
                                end
                            end
                            ...
                            TriggerServerEvent('x-vehicletuning:server:UpdateDrivingDistance', DrivingDistance[plate], plate)
                        end
                    else
                        if invehicle then
                            if DrivingDistance[plate] ~= nil then
...
RegisterNetEvent('x-vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('x-vehicletuning:client:UpdateDrivingDistance', function(amount, plate)

    ...
    XCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("XCore:NotifyToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    XCore.Functions.TriggerCallback('x-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    XCore.Functions.TriggerCallback('x-vehicletuning:server:GetDrivingDistances', function(retval)
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, 38) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('x-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            XCore.Functions.Notify("You Cannot Put Bicycles On The Platform!", "error")
                                        end
                                    end
                                end
                            end
...
function SpawnListVehicle(model)
    ...
    XCore.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..XCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    XCore.Functions.TriggerCallback('x-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    XCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('x-vehicletuning:client:RepaireeePart', part)
                        TriggerServerEvent('x-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                        SetTimeout(250, function()
                            PartsMenu()
                        end)
                    end, function()
                        XCore.Functions.Notify("Repair Canceled", "error")
                    end)
                    break
                else
                    XCore.Functions.Notify('There Are Not Enough Materials In The Safe', 'error')
                end
                break
            else
                XCore.Functions.Notify('There Are Not Enough Materials In The Safe', 'error')
...
RegisterNetEvent('x-vehicletuning:client:RepaireeePart')
AddEventHandler('x-vehicletuning:client:RepaireeePart', function(part)
    ...
    XCore.Functions.Notify("The "..Config.ValuesLabels[part].." Is Repaired!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('x-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('x-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('x-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        XCore.Functions.Notify("Status Unknown", "error")
                    end
                else
                    XCore.Functions.Notify("Not A Valid Vehicle", "error")
                end
            else
                XCore.Functions.Notify("You Are Not Close Enough To The Vehicle", "error")
            end
        else
            XCore.Functions.Notify("You Must Be In The Vehicle First", "error")
        end
    else
        XCore.Functions.Notify("You Must Be Outside The Vehicle", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            XCore.Functions.Notify("You Are Not The Driver Or On A Bicycle", "error")
        end
    else
...
AddEventHandler('vehiclemod:client:repairPart', function(part, level, needAmount)
    -- if CanReapair() then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            XCore.Functions.Progressbar("repair_advanced", "Repair Vehicle", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("XCore:NotifyServer:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", XCore.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("XCore:NotifyServer:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", XCore.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                            ...
                            XCore.Functions.Notify("Not A Valid Part", "error")
                        end
                    else
                        XCore.Functions.Notify("Not A Valid Vehicle", "error")
                    end
            ...
            XCore.Functions.Notify("Youre Not In a Vehicle", "error")
        end
    -- end
end)
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['x-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..XCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..XCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..XCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..XCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..XCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..XCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..XCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end

...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    isLoggedIn = true
    PlayerData = XCore.Functions.GetPlayerData()
...
function StartWorking()
    XCore.Functions.TriggerCallback('x-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = PlayerPedId()
            ...
            UpdateUI()
            CheckLoop()
            XCore.Functions.Notify('You have $250 deposit paid!..', 'success')
        else
            XCore.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
RegisterNetEvent('x-hotdogjob:client:UpdateReputation')
AddEventHandler('x-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('x-hotdogjob:client:ToggleSell')
AddEventHandler('x-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            XCore.Functions.Notify('You are to far from your Hot Dog Stand..', 'error')
        end
    else
        XCore.Functions.Notify('You do not have a hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
	    ...
	    SellingData.Hotdog = nil
            XCore.Functions.Notify('You are too far from your stall ...', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 4 then
            if SellingData.Hotdog ~= nil then
                ...
                XCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x in front of $'..(HotdogsForSale * SellingPrice)..' / [8] Reject')
                if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then
                    XCore.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold in front of $'..(HotdogsForSale * SellingPrice), 'success')
                    TriggerServerEvent('x-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    XCore.Functions.Notify('Customer Refused!', 'error')
                    SellingData.HasTarget = false
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('x-keyminigame:show')
    TriggerEvent('x-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('x-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            XCore.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hot Dog Made!')
            ...
            if Luck == LuckyNumber then
                XCore.Functions.Notify('You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                XCore.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hotdog made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        XCore.Functions.Notify('You have no ('..Config.Stock[Quality].Label..') more about this in front of council..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        XCore.Functions.TriggerCallback('x-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                XCore.Functions.Notify('You have a $250 deposit returned!', 'success')
            else
                XCore.Functions.Notify('Something is my friend..', 'error')
            end
        end)
    else
        XCore.Functions.Notify('Your hot dog stand was nowhere to be seen, You will not receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
RegisterNetEvent('x-hotdogjob:staff:DeletStand')
AddEventHandler('x-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            XCore.Functions.Notify('Hotdogstand mode removed!')
        end
    end
end)

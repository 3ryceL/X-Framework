...
RegisterNetEvent('x-drugs:client:cornerselling')
AddEventHandler('x-drugs:client:cornerselling', function(data)
    XCore.Functions.TriggerCallback('x-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            ...
            if not cornerselling then
                ...
                XCore.Functions.Notify('You started selling drugs')
                startLocation = GetEntityCoords(PlayerPedId())
                -- TaskStartScenarioInPlace(PlayerPedId(), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                LocalPlayer.state:set("inv_busy", false, true)
                XCore.Functions.Notify('You stopped selling drugs')
                -- ClearPedTasks(PlayerPedId())
            end
        else
            XCore.Functions.Notify('You aren\'t carrying any weed with you..', 'error')
            LocalPlayer.state:set("inv_busy", false, true)
        end
    end)
end)
...
function toFarAway()
    XCore.Functions.Notify('Moved Too Far!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(ped)
                        TriggerServerEvent("XCore:NotifyServer:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('x-drugs:client:refreshAvailableDrugs')
AddEventHandler('x-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        XCore.Functions.Notify('No more drugs left to sell!', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('x-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                XCore.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    XCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('x-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, 47) then
                        XCore.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false

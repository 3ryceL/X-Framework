...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    XCore.Functions.TriggerCallback('x-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        XCore.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            XCore.Functions.Notify("You have helped a person!")
                                        ...
                                        XCore.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if XCore.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = XCore.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        XCore.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end

RegisterNetEvent('x-drugs:client:updateDealerItems')
AddEventHandler('x-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('x-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('x-drugs:client:setDealerItems')
AddEventHandler('x-drugs:client:setDealerItems', function(itemData, amount, dealer)
...
function requestDelivery()
    ...
    TriggerServerEvent('x-drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('x-phone:server:sendNewMail', {
            sender = Config.Dealers[currentDealer]["name"],
            subject = "Delivery Location",
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..XCore.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be in time!!",
            button = {
                enabled = true,
                buttonEvent = "x-drugs:client:setLocation",
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = XCore.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    XCore.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end

RegisterNetEvent('x-drugs:client:setLocation')
AddEventHandler('x-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        XCore.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..XCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        XCore.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('x-drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
            XCore.Functions.Notify("Canceled..", "error")
        end)
    else
        TriggerServerEvent('x-drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function checkPedDistance()
    ...
    local closestPed, closestDistance = XCore.Functions.GetClosestPed(coords, PlayerPeds)
...
function doPoliceAlert()
    ...
    TriggerServerEvent('x-drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('x-drugs:client:robberyCall')
AddEventHandler('x-drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('x-drugs:client:CreateDealer')
AddEventHandler('x-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent('x-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('x-drugs:client:RefreshDealers')
AddEventHandler('x-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('x-drugs:client:GotoDealer')
AddEventHandler('x-drugs:client:GotoDealer', function(DealerData)
    local ped = PlayerPedId()

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    XCore.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)

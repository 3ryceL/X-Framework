...
Citizen.CreateThread(function()
    ...
    if XCore.Functions.GetPlayerData() ~= nil then
        ...
        XCore.Functions.TriggerCallback('x-traphouse:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('x-traphouse:client:EnterTraphouse')
AddEventHandler('x-traphouse:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        XCore.Functions.Notify('This Code Is Incorrect', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('x-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent('x-traphouse:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, 47) then
                                    XCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, 74) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent("x-traphouse:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['x-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('x-traphouse:client:TakeoverHouse')
AddEventHandler('x-traphouse:client:TakeoverHouse', function(TraphouseId)
    local ped = PlayerPedId()

    XCore.Functions.Progressbar("takeover_traphouse", "Taking Over", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('x-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        XCore.Functions.Notify("Acquisitions Canceled", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            XCore.Functions.Notify(CitizenId..' Has Been Added To The Traphouse!')
        else
            XCore.Functions.Notify(CitizenId..' This Person Already Has Keys')
        end
    else
        XCore.Functions.Notify('You Can Give Up To 6 People Access To The Trap House!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('x-traphouse:client:SyncData')
AddEventHandler('x-traphouse:client:SyncData', function(k, data)

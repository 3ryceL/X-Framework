RegisterServerEvent('x-traphouse:server:TakeoverHouse')
AddEventHandler('x-traphouse:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('x-traphouse:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('x-traphouse:server:AddHouseKeyHolder')
AddEventHandler('x-traphouse:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            TriggerClientEvent('x-traphouse:client:SyncData', -1, TraphouseId, Config.TrapHouses[TraphouseId])
        else
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('XCore:NotifyNotify', src, 'There Are No Slots Left')
            end
        end
    else
        TriggerClientEvent('XCore:NotifyNotify', src, 'Error Has Occurred')
    end
end)
...
XCore.Commands.Add("entertraphouse", "Betreed traphouse", {}, false, function(source, args)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
 
    TriggerClientEvent('x-traphouse:client:EnterTraphouse', src)
end)

XCore.Commands.Add("multikeys", "Give Keys To Traphouse", {{name = "id", help = "Player id"}}, true, function(source, args)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('x-traphouse:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                ...
                TriggerClientEvent('XCore:NotifyNotify', src, 'This Person Already Has Keys', 'error')
            end
        else
            TriggerClientEvent('XCore:NotifyNotify', src, 'You Do Not Own A Traphouse Or Are Not The Owner', 'error')
        end
    else
        TriggerClientEvent('XCore:NotifyNotify', src, 'This Person Is Not In The City', 'error')
    end
end)

RegisterServerEvent('x-traphouse:server:TakeMoney')
AddEventHandler('x-traphouse:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('XCore:NotifyNotify', src, 'There issent any money in the cupboard', 'error')
    end
end)
...
RegisterServerEvent('x-traphouse:server:RobNpc')
AddEventHandler('x-traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = XCore.Functions.GetPlayer(src)
...
XCore.Functions.CreateCallback('x-traphouse:server:GetTraphousesData', function(source, cb)

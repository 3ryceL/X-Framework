...
RegisterNetEvent('x-admin:client:openMenu')
AddEventHandler('x-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do 
        ...
        TriggerServerEvent('x-weathersync:server:setWeather', selection.value)
            XCore.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    XCore.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("x-weathersync:server:setTime", value, value)
    XCore.Functions.Notify("Time changed to " .. value .. " hs 00 min")
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('x-admin:server:'..values, player)
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('x-admin:server:ban', banspeler, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                XCore.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('x-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                XCore.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
...
function OpenPermsMenu(permsply)
    XCore.Functions.TriggerCallback('x-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('x-admin:server:setPermissions', permsply.id, selectedgroup)
                        selectedgroup = 'Unknown'
                    else
                        XCore.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                XCore.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete the object!", 2)

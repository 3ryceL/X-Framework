...
AddEventHandler("XCore:NotifyClient:OnPlayerLoaded", function()
    TriggerServerEvent("x-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										XCore.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									XCore.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
...
RegisterNetEvent('x-scapyard:client:setNewVehicles')
AddEventHandler('x-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = XCore.Shared.Vehicles[v]
		...
		XCore.Functions.Notify("You Are Not Allowed To Demolish Vehicles Now", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	XCore.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
		...
		TriggerServerEvent("x-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))

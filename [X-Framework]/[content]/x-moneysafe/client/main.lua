...
RegisterNetEvent('x-moneysafe:client:DepositMoney')
AddEventHandler('x-moneysafe:client:DepositMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('x-moneysafe:server:DepositMoney', ClosestSafe, amount)
            end
        end
    end
end)

RegisterNetEvent('x-moneysafe:client:WithdrawMoney')
AddEventHandler('x-moneysafe:client:WithdrawMoney', function(amount)
    if ClosestSafe ~= nil then
        if IsAuthorized then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)
            ...
            if distance < Config.MinimumSafeDistance then
                TriggerServerEvent('x-moneysafe:server:WithdrawMoney', ClosestSafe, amount)
            end
        end
    end
end)
...
RegisterNetEvent('x-moneysafe:client:UpdateSafe')
AddEventHandler('x-moneysafe:client:UpdateSafe', function(SafeData, k)

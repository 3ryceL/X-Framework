...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    XCore.Functions.TriggerCallback('x-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = XCore.Shared.Items["lockpick"]["name"], image = XCore.Shared.Items["lockpick"]["image"]},
        [2] = {name = XCore.Shared.Items["screwdriverset"]["name"], image = XCore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['x-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('x-houserobbery:client:ResetHouseState')
AddEventHandler('x-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('x-houserobbery:client:enterHouse')
AddEventHandler('x-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        PoliceCall()
                        TriggerEvent('x-lockpick:client:openLockpick', lockpickFinish)
                        if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                            local pos = GetEntityCoords(PlayerPedId())
                            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
                        end
                    else
                        XCore.Functions.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    XCore.Functions.Notify('Not enough Police..', 'error', 3500)
                end
            end
        else
            XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        XCore.Functions.Notify('It looks like you are missing something...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('x-houserobbery:server:enterHouse', closestHouse)
        XCore.Functions.Notify('It worked!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = XCore.Shared.Items["advancedlockpick"]
            ...
            local itemInfo = XCore.Shared.Items["lockpick"]
        ...
        XCore.Functions.Notify('It did not work..', 'error', 2500)
    end
end

RegisterNetEvent('x-houserobbery:client:setHouseState')
AddEventHandler('x-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('x-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('x-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('x-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('x-houserobbery:client:setCabinState')
AddEventHandler('x-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('x-houserobbery:client:SetBusyState')
AddEventHandler('x-houserobbery:client:SetBusyState', function(cabin, house, bool)

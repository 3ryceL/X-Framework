...
$(document).on("dblclick", ".item-slot", function(e){
    ...
    if(ItemData) {
        Inventory.Close();
        $.post("https://x-inventory/UseItem", JSON.stringify({
...
function FormatAttachmentInfo(data) {
    $.post("https://x-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('https://x-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('https://x-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("https://x-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('https://x-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('https://x-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("https://x-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("https://x-inventory/PlayDropSound", JSON.stringify({}));
            $.post("https://x-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('https://x-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://x-inventory/PlayDropFail", JSON.stringify({}));
}
...
/*             "weapon_pistol_mk2", 
    ...
    Inventory.Close = function() {
        ...
        $("#x-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://x-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#x-inventory").css({"left": "0vw"});
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('https://x-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});

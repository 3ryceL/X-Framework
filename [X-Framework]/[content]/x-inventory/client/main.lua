...
AddEventHandler('inventory:client:CheckOpenState', function(type, id, label)
    local name = XCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
        ...
        if showTrunkPos and not inInventory then
            ...
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
                    ...
                    XCore.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Trunk")
                    ...
                    if vehicle ~= 0 and vehicle ~= nil then
                        ...
                        if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                            if GetVehicleDoorLockStatus(vehicle) < 2 then
                                ...
                                XCore.Functions.Notify("Vehicle is locked..", "error")
                                return
                            end
                        else
                            CurrentVehicle = nil
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = XCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
RegisterNUICallback('Notify', function(data, cb)
    XCore.Functions.Notify(data.message, data.type)
end)
...
AddEventHandler("inventory:client:OpenInventory", function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = XCore.Config.Player.MaxWeight,
            Ammo = PlayerAmmo,
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = XCore.Functions.GetPlayerData().items,
        maxweight = XCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    XCore.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    XCore.Functions.Progressbar("pickupsnowball", "Sneeuwballen oprapen..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('XCore:NotifyServer:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(ped)
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_stickybomb" then
        ...
        TriggerServerEvent('XCore:NotifyServer:RemoveItem', weaponName, 1)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        XCore.Functions.TriggerCallback("weapon:server:GetWeaponAmmo", function(result)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = XCore.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local ped = PlayerPedId()
    local WeaponData = XCore.Shared.Items[data.WeaponData.name]
    local Attachment = WeaponAttachments[WeaponData.name:upper()][data.AttachmentData.attachment]
    
    XCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(XCore.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    XCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', XCore.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(ped, aDict, aLib, 1.0)
        XCore.Functions.Notify("Mislukt!", "error")
    end)
end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = XCore.Functions.GetPlayerData().items[1],
        [2] = XCore.Functions.GetPlayerData().items[2],
        [3] = XCore.Functions.GetPlayerData().items[3],
        [4] = XCore.Functions.GetPlayerData().items[4],
        [5] = XCore.Functions.GetPlayerData().items[5],
        [41] = XCore.Functions.GetPlayerData().items[41],
    } 

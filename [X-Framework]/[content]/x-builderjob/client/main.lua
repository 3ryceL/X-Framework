...
function GetCurrentProject()
    XCore.Functions.TriggerCallback('x-builderjob:server:GetCurrentProject', function(BuilderConfig)
                    ...
                    if TaskData ~= nil then
                        ...
                        if TaskData.completed == TaskData.total then
                            ...
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent('x-builderjob:server:FinishProject')
                            end
                        end
...
function DoTask()
    ...
    local CountDown = 5
    TriggerServerEvent('x-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, false)
    ...
    if TaskData.type == "hammer" then
        ...
        TriggerServerEvent('x-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, true)
    end
end

RegisterNetEvent('x-builderjob:client:SetTaskState')
AddEventHandler('x-builderjob:client:SetTaskState', function(Task, IsBusy, IsCompleted)
...
RegisterNetEvent('x-builderjob:client:FinishProject')
AddEventHandler('x-builderjob:client:FinishProject', function(BuilderConfig)
    Config = BuilderConfig
end)
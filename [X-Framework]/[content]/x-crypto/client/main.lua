...
Citizen.CreateThread(function()
	while true do
		...
		if XCore ~= nil then
			local requiredItems = {
				[1] = {name = XCore.Shared.Items["cryptostick"]["name"], image = XCore.Shared.Items["cryptostick"]["image"]},
			}
			if isLoggedIn then
					...
					if dist < 1.5 then
						if not Crypto.Exchange.RebootInfo.state then
							...
							if IsControlJustPressed(0, 38) then
								XCore.Functions.TriggerCallback('x-crypto:server:HasSticky', function(HasItem)
									if HasItem then
										TriggerEvent("mhacking:show")
										TriggerEvent("mhacking:start", math.random(4, 6), 45, HackingSuccess)
									else
										XCore.Functions.Notify('You have no Cryptostick ..', 'error')
									end
								end)
							end
						else
...
function ExchangeSuccess()
	TriggerServerEvent('x-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('x-crypto:server:ExchangeFail')
		TriggerServerEvent('x-crypto:server:SyncReboot')
		-- Crypto.Exchange.RebootInfo.state = true
		-- SystemCrashCooldown()
	end
end

RegisterNetEvent('x-crypto:client:SyncReboot')
AddEventHandler('x-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)
	...
	Citizen.CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			...
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('x-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('x-crypto:server:Rebooting', false, 0)
			end
...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
	isLoggedIn = true
	TriggerServerEvent('x-crypto:server:FetchWorth')
	TriggerServerEvent('x-crypto:server:GetRebootState')
end)

RegisterNetEvent('x-crypto:client:UpdateCryptoWorth')
AddEventHandler('x-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('x-crypto:client:GetRebootState')
AddEventHandler('x-crypto:client:GetRebootState', function(RebootInfo)

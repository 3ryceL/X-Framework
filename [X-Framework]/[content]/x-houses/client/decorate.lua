	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				XCore.Functions.Notify("You have gone out of range!")
			end
		end
	end
end)
...
RegisterNetEvent("x-houses:client:decorate")
AddEventHandler("x-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			XCore.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNetEvent("x-houses:server:sethousedecorations")
AddEventHandler("x-houses:server:sethousedecorations", function(house, decorations)
...
function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('x-anticheat:client:ToggleDecorate', true)
end
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('x-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		XCore.Functions.TriggerCallback('x-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("x-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		XCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end

...
AddEventHandler('jobs:client:ToggleNpc', function()
    if XCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            XCore.Functions.Notify("First Finish Your Work", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('x-tow:client:TowVehicle')
AddEventHandler('x-tow:client:TowVehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "XCore.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 XCore native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    XCore.Functions.Notify("This Is Not The Right Vehicle", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        XCore.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                XCore.Functions.Notify("Take The Vehicle To Hayes Depot", "success", 5000)
                                ...
                                if chance < 26 then
                                    TriggerServerEvent('x-tow:server:nano')
                                end
                            end
                            XCore.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            XCore.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                XCore.Functions.Notify("Vehicle Taken Off")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                XCore.Functions.Notify("Failed", "error")
            end)
        ...
        XCore.Functions.Notify("You Must Have Been In A Towing Vehicle First", "error")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(1)
        if isLoggedIn and XCore ~= nil then
            if PlayerJob.name == "tow" then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                ...
                                TriggerServerEvent('x-tow:server:DoBail', false)
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                RemoveBlip(CurrentBlip)
                                TriggerServerEvent("x-tow:server:11101110", JobsDone)
                                JobsDone = 0
                                NpcOn = false
                            else
                                XCore.Functions.Notify("You Havent Done Any Work Yet", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        XCore.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    XCore.Functions.Notify("You Have Delivered A Vehicle", "success")
    XCore.Functions.Notify("A New Vehicle Can Be Picked Up")
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('x-tow:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end

RegisterNetEvent('x-tow:client:SpawnVehicle')
AddEventHandler('x-tow:client:SpawnVehicle', function()

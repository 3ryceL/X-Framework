        ...
        if distance < 6 then
            ...
            if distance < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("x-coke:server:grindleaves")
    ...
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("x-coke:server:processCrack")
                end
            end
...
RegisterNetEvent('x-coke:client:grindleavesMinigame')
AddEventHandler('x-coke:client:grindleavesMinigame', function(source)
    PrepareProcessAnim()
    ProcessMinigame(source)
end)

RegisterNetEvent('x-coke:client:processCrack')
AddEventHandler('x-coke:client:processCrack', function(source)
    ProcessCrackMinigame(source)
end)

function pickProcess()
    XCore.Functions.Progressbar("grind_coke", "Picking Coca Leaves ..", math.random(80000,120000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("x-coke:server:getleaf")
...
function cokeProcess()
    XCore.Functions.Progressbar("grind_coke", "Process Coke Leaves ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("x-coke:server:getcoke")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function crackProcess()
    XCore.Functions.Progressbar("grind_coke", "Process Crack ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("x-coke:server:getcrack")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            XCore.Functions.Notify("You picked a coca leaf", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            XCore.Functions.Notify("You messed up the coca leaf!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokepicking = false
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            XCore.Functions.Notify("You make some coke!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            XCore.Functions.Notify("You messed up the process!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokeprocess = false
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            XCore.Functions.Notify("You make some crack!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        TriggerServerEvent("x-coke:server:cokesell")
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        XCore.Functions.Notify('It seems that no one is home..', 'error', 3500)
    end
end

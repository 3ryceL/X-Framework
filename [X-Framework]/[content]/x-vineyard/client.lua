							...
							DrawText3Ds(-1928.81, 2059.53, 140.84, "[E] Start Picking Grapes") 
								...
								if IsControlJustReleased(0,38) then
									if PlayerJob.name == "vineyard" then
										...
										XCore.Functions.Notify("I dont think I work here...", "error")
									end
								end
						end
					end
	...
	while true do
		if startVineyard then
			if tasking then
				...
				TriggerEvent("x-vineyard:client:startVineyard")
...
RegisterNetEvent('x-vineyard:client:startVineyard')
AddEventHandler('x-vineyard:client:startVineyard', function()
...
function pickgrapes()
	...
	if success then
		TriggerServerEvent("x-vineyard:server:getGrapes")
		tasking = false
	end
end
...
function pickProcess()
    XCore.Functions.Progressbar("pick_grape", "Picking Grapes ..", math.random(6000,8000), false, true, {
							...
							if not loadIngredients then
								if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
									...
									if IsControlJustPressed(0, 38) then
										if PlayerJob.name == "vineyard" then
											TriggerServerEvent("x-vineyard:server:loadIngredients")
										else
											XCore.Functions.Notify("I dont think I work here...", "error")
										end
									end
								...
								if not finishedWine then
									...
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										...
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
												TriggerServerEvent("x-vineyard:server:receiveWine")
												finishedWine = false
			...
			if nearlocation <= 15 then
				...
				if nearlocation <= 3 then	
					...
					if #(pos - vector3(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y, Config.Vineyard["grapejuice"].coords.z)) < 1 then
						...
						if IsControlJustPressed(0, 38) then
							if PlayerJob.name == "vineyard" then
								TriggerServerEvent("x-vineyard:server:grapeJuice")
							else
								XCore.Functions.Notify("I dont think I work here...", "error")
							end
						end
...
RegisterNetEvent('x-vineyard:client:grapeJuice')
AddEventHandler('x-vineyard:client:grapeJuice', function()
	PrepareAnim()
	grapeJuiceProcess()
end)

RegisterNetEvent('x-vineyard:client:loadIngredients')
AddEventHandler('x-vineyard:client:loadIngredients', function()
...
function grapeJuiceProcess()
    XCore.Functions.Progressbar("grape_juice", "Processing Grapes ..", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("x-vineyard:server:receiveGrapeJuice")
        ClearPedTasks(PlayerPedId())

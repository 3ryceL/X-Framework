...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("x-methlab:server:LoadLocationList")
end)
...
Citizen.CreateThread(function()
    Wait(500)
    XCore.Functions.TriggerCallback('x-methlab:server:GetData', function(data)
            ...
            if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 1 then
                    ...
                    if IsControlJustPressed(0, 47) then
                        TriggerServerEvent("x-methlab:server:breakMeth")
                    end
                end
            end
        end
                ...
                if  not machineStarted then
                    if not loadIngredients then
                        if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                            ...
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent("x-methlab:server:loadIngredients")
                            end
                        end
                    else
                        if not finishedMachine then
                            ...
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent("x-methlab:server:receivemethtray")
...
RegisterNetEvent('x-methlab:client:breakMeth')
AddEventHandler('x-methlab:client:breakMeth', function()
    PrepareAnim()
    BreakMinigame()
end)

RegisterNetEvent('x-methlab:client:loadIngredients')
AddEventHandler('x-methlab:client:loadIngredients', function()
    ProcessMinigame()
end)
...
function breakingMeth(amount)
    XCore.Functions.Progressbar("break_meth", "Breaking Meth ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("x-methlab:server:getmethtray", amount)
        ClearPedTasks(PlayerPedId())
    end, function() -- Cancel
    end)
end
...
function ProcessMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            XCore.Functions.Notify("You loaded the ingredients!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
...
RegisterNetEvent('x-methlab:client:UseLabKey')
AddEventHandler('x-methlab:client:UseLabKey', function(labkey)
    if ClosestMethlab == Config.CurrentLab then
        ...
        if dist < 1 then
            if labkey == ClosestMethlab then
                ...
                XCore.Functions.Notify('This is not the correct key..', 'error')
            end
        end
    end
end)
...
function PoliceCall(vehicle)
    ...
    if XCore.Shared.Vehicles[vehname] ~= nil then
        Name = XCore.Shared.Vehicles[vehname]["model"]["brand"] .. ' ' .. XCore.Shared.Vehicles[vehname]["model"]["name"]
    else
        Name = "Unknown"
    end
	...
	if MethRun then
        ...
        SetTimeout(2000, function()
            TriggerServerEvent('x-phone:server:sendNewMail', {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "x-methlab:client:setLocation",
...
RegisterNetEvent('x-methlab:client:setLocation')
AddEventHandler('x-methlab:client:setLocation', function(locationData)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			if MethChance <= Config.BigRewarditemChance then
				TriggerServerEvent("x-methlab:server:receiveBigRewarditem")
            else
                TriggerServerEvent("x-methlab:server:receivemeth")
			end

			Citizen.Wait(2000)

			XCore.Functions.Notify('The delivery was on point, your pager will be updated with the next drop off', 'success', 2500)
		else
			XCore.Functions.Notify('The drop off failed', 'error', 2500)
		end
...
RegisterNetEvent("x-methlab:client:client")
AddEventHandler("x-methlab:client:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (methVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateMethPed()
			XCore.Functions.Notify("You are close to the drop off")
		end
		toolong = toolong - 1
		if toolong <= 0 then
			...
			tasking = false
			MethRun = false
			XCore.Functions.Notify('You are no longer selling meth due to taking too long to drop off', 'error', 2500)
			DeleteBlip()
		end
...
RegisterNetEvent('x-drugs:client:sendDeliveryMail')
AddEventHandler('x-drugs:client:sendDeliveryMail', function(type, deliveryData)
    ...
    while true do
		...
		if dropOff6 < 1.6 and not MethRun then
			...
			if IsControlJustReleased(0,38) then
				TriggerServerEvent("x-methlab:server:server")
				Citizen.Wait(1000)
			end
		end
    ...
    while true do
		...
		elseif MethRun then
			...
			if not DoesEntityExist(methVehicle) or GetVehicleEngineHealth(methVehicle) < 200.0 or GetVehicleBodyHealth(methVehicle) < 200.0 then
				MethRun = false
				tasking = false
				XCore.Functions.Notify('The dealer isnt giving you anymore locations due to the state of his car', 'error', 3500)
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
			        TriggerEvent("x-methlab:client:client")  
				    salecount = salecount + 1
					if salecount == Config.RunAmount then
						XCore.Functions.Notify('You are on your last delivery!', 'error', 3500)
				    	Citizen.Wait(420000)
						MethRun = false
				    end
				end
...
RegisterNetEvent("x-methlab:client:startDealing")
AddEventHandler("x-methlab:client:startDealing", function()
	...
	MethRun = true
	firstdeal = true
	XCore.Functions.Notify('A car has been left outside for you. Your pager will be updated with locations soon', 'success', 2500)
	CreateMethVehicle()
end)

function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    XCore.Functions.Notify('The delivery location is marked on your GPS.', 'success');
end
...
--                                     TriggerServerEvent('x-methlab:server:PoliceAlertMessage', 'People try to steal items at the Pharmacy in Palteo', pos, true)
--                                         currentSpot = spot
--                                         GrabItem(currentSpot)
--                                 else
--                                     XCore.Functions.Notify("Not enough Police", "error")
--                                 end
--                             end
--                         end
--                     end
...
--     local pos = GetEntityCoords(PlayerPedId())
--     XCore.Functions.Progressbar("grab_pills", "Grabbing Pills", 5000, false, true, {
--         disableMovement = true,
--         disableCarMovement = true,
--         disableMouse = false,
--         disableCombat = true,
...
--         TriggerServerEvent('x-methlab:server:setSpotState', "isDone", true, spot)
--         TriggerServerEvent('x-methlab:server:setSpotState', "isBusy", false, spot)
--         TriggerServerEvent('x-methlab:server:itemReward', spot)
--     end, function() -- Cancel
--         StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
--         TriggerServerEvent('x-methlab:server:setSpotState', "isBusy", false, spot)
...
-- RegisterNetEvent('x-methlab:client:setSpotState')
-- AddEventHandler('x-methlab:client:setSpotState', function(stateType, state, spot)
...
-- RegisterNetEvent('x-methlab:client:PoliceAlertMessage')
-- AddEventHandler('x-methlab:client:PoliceAlertMessage', function(msg, coords, blip)
--     if blip then

...
if ActiveMission == 0 then
	...
	TriggerClientEvent('XCore:NotifyNotify', _source, "You need $"..ActivationCost.." in the bank to accept the mission")
	else
	...
	if copsOnDuty >= ActivePolice then
		...
		TriggerClientEvent('XCore:NotifyNotify', _source, 'Need at least '..ActivePolice.. ' SASP to activate the mission.')
    end
	end
else
TriggerClientEvent('XCore:NotifyNotify', _source, 'Someone is already carrying out this mission')
end
end)

RegisterServerEvent('x-armoredtruckheist:server:callCops')
AddEventHandler('x-armoredtruckheist:server:callCops', function(streetLabel, coords)
    local place = "Armored Truck"
    local msg = "The Alram has been activated from a "..place.. " at " ..streetLabel

    TriggerClientEvent("x-armoredtruckheist:client:robberyCall", -1, streetLabel, coords)
...
AddEventHandler('AttackTransport:graczZrobilnapad', function(moneyCalc)
	...
	TriggerClientEvent('inventory:client:ItemBox', _source, XCore.Shared.Items['markedbills'], "add")

	local chance = math.random(1, 100)
	TriggerClientEvent('XCore:NotifyNotify', _source, 'You took '..bags..' bags of cash from the van')

	if chance >= 95 then
	xPlayer.Functions.AddItem('security_card_01', 1)
	TriggerClientEvent('inventory:client:ItemBox', _source, XCore.Shared.Items['security_card_01'], "add")
	end

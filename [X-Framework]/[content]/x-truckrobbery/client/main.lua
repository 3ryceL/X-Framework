...
RegisterNetEvent('x-armoredtruckheist:client:911alert')
AddEventHandler('x-armoredtruckheist:client:911alert', function()
	if PoliceAlert == 0 then
		...
		if street2 ~= nil then 
			...
			TriggerServerEvent("x-armoredtruckheist:server:callCops", streetLabel, transCoords)
...
RegisterNetEvent('x-armoredtruckheist:client:robberyCall')
AddEventHandler('x-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
    ...
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
				warning = 1
				XCore.Functions.Notify("Get rid of the guards before you place the bomb.", "error")
				end
			...
			if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
				...
				if IsControlPressed(0, 47) and GuardsDead == 1 then 
					...
					TriggerEvent("x-armoredtruckheist:client:911alert")
					Citizen.Wait(500)
				end
			end
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			XCore.Functions.Notify('The load will be detonated in '..TimeToBlow / 1000 ..' seconds.', "error")
			...
			BlownUp = 1
			lootable = 1
			XCore.Functions.Notify('You can start collecting cash.', "success")
			RemoveBlip(TruckBlip)
		else
			XCore.Functions.Notify('Get out of the water', "error")
		end
	else
		XCore.Functions.Notify('The vehicle must be empty to place the load', "error")
	end
else
	XCore.Functions.Notify('You cant rob a vehicle that is moving.', "error")
end
end
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	XCore.Functions.Notify('You are packing cash into a bag', "success")

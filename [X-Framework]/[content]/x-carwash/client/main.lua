                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('x-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('x-carwash:client:washCar')
AddEventHandler('x-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    XCore.Functions.Progressbar("search_cabin", "Vehicle is being washed ..", math.random(4000, 8000), false, true, {
        ...
        XCore.Functions.Notify("Washing canceled ..", "error")
        washingVehicle = false
    end)
end)

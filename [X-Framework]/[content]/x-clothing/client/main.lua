...
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    TriggerServerEvent("x-clothes:loadPlayerSkin")
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    gender = "male"
                                    if XCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    XCore.Functions.TriggerCallback('x-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('x-clothing:client:openOutfitMenu')
AddEventHandler('x-clothing:client:openOutfitMenu', function()
    XCore.Functions.TriggerCallback('x-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('x-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('x-clothing:client:openMenu')
AddEventHandler('x-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('TrackerError', function()
    XCore.Functions.Notify("You can't remove your ankle bracelet ..", "error")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('x-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('x-clothing:client:reloadOutfits')
AddEventHandler('x-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('x-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    XCore.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
end)
...
function SaveSkin()
	...
	TriggerServerEvent("x-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('x-clothes:client:CreateFirstCharacter')
AddEventHandler('x-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("x-clothes:loadSkin")
AddEventHandler("x-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('x-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('x-clothing:client:loadPlayerClothing')
AddEventHandler('x-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('x-clothing:client:loadOutfit')
AddEventHandler('x-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if XCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        XCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
...
RegisterNetEvent("x-clothing:client:adjustfacewear")
AddEventHandler("x-clothing:client:adjustfacewear",function(type)
    if XCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end

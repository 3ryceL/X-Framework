...
XClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://x-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://x-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://x-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                XClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    XClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://x-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://x-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://x-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://x-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
XClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                XClothing.Open(event.data);
                break;
            case "close":
                XClothing.Close();
                break;
            case "updateMax":
                XClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                XClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                XClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                XClothing.ResetValues();
                break;
...
XClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    XClothing.Close();
    $.post('https://x-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    XClothing.Close();
    $.post('https://x-clothing/resetOutfit');
});

XClothing.SetCurrentValues = function(clothingValues) {
...
XClothing.Open = function(data) {
    ...
    XClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    XClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://x-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://x-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
XClothing.Close = function() {
    $.post('https://x-clothing/close');
...
XClothing.SetMaxValues = function(maxValues) {
...
XClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://x-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://x-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});

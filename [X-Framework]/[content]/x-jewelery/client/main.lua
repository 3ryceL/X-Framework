                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        XCore.Functions.TriggerCallback('x-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    XCore.Functions.Notify('Your weapon is not strong enough..', 'error')
                                                end
                                            else
                                                XCore.Functions.Notify('There are not enough police', 'error')
                                            end                
                                        end)
                                    end
                                end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        XCore.Functions.Notify("You've left a fingerprint on the glass", "error")
    end

    smashing = true

    XCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('x-jewellery:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('x-jewellery:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('x-jewellery:server:vitrineReward')
        TriggerServerEvent('x-jewellery:server:setTimeout')
    ...
    TriggerServerEvent('x-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('x-jewellery:client:setVitrineState')
AddEventHandler('x-jewellery:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('x-jewellery:client:setAlertState')
AddEventHandler('x-jewellery:client:setAlertState', function(bool)
    robberyAlert = bool
end)

RegisterNetEvent('x-jewellery:client:PoliceAlertMessage')
AddEventHandler('x-jewellery:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then

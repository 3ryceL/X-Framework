...
RegisterNetEvent('x-doorlock:client:setState')
AddEventHandler('x-doorlock:client:setState', function(doorID, state)
	X.Doors[doorID].locked = state
end)

RegisterNetEvent('x-doorlock:client:setDoors')
AddEventHandler('x-doorlock:client:setDoors', function(doorList)
	X.Doors = doorList
end)

RegisterNetEvent('XCore:NotifyClient:OnPlayerLoaded')
AddEventHandler('XCore:NotifyClient:OnPlayerLoaded', function()
    TriggerServerEvent("x-doorlock:server:setupDoors")
end)
...
AddEventHandler('lockpicks:UseLockpick', function()
	...
	XCore.Functions.TriggerCallback('XCore:NotifyHasItem', function(hasItem)
		for k, v in pairs(X.Doors) do
			local dist = #(pos - vector3(X.Doors[k].textCoords.x, X.Doors[k].textCoords.y, X.Doors[k].textCoords.z))
			if dist < 1.5 then
				if X.Doors[k].pickable then
					if X.Doors[k].locked then
						if hasItem then
							closestDoorKey, closestDoorValue = k, v
							TriggerEvent('x-lockpick:client:openLockpick', lockpickFinish)
						else
							XCore.Functions.Notify("You Dont Have A Screwdriver Set", "error")
						end
					else
						XCore.Functions.Notify('The Door Is Not Locked', 'error', 2500)
					end
				else
					XCore.Functions.Notify('This Door Can Not Be Lockpicked', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		XCore.Functions.Notify('Success!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        XCore.Functions.Notify('Failed', 'error', 2500)
    end
end
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('x-doorlock:server:updateState', key, doorId.locked)
	end)
end
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						XCore.Functions.Notify('Not Authorized', 'error')
					end
				end
			end
		end

    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('https://x-multicharacter/setupCharacters');
                    setTimeout(function(){
                        clearInterval(DotsInterval);
                        loadingProgress = 0;
                        originalText = "Retrieving data";
                        $(".welcomescreen").fadeOut(150);
                        xMultiCharacters.fadeInDown('.character-info', '20%', 400);
                        xMultiCharacters.fadeInDown('.characters-list', '20%', 400);
                        $.post('https://x-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.continue-btn').click(function(e){
    ...
    // xMultiCharacters.fadeOutUp('.welcomescreen', undefined, 400);
    // xMultiCharacters.fadeOutDown('.server-log', undefined, 400);
    // setTimeout(function(){
    //     xMultiCharacters.fadeInDown('.characters-list', '20%', 400);
    //     xMultiCharacters.fadeInDown('.character-info', '20%', 400);
    //     $.post('https://x-multicharacter/setupCharacters');
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://x-multicharacter/closeUI');
    $.post('https://x-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://x-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function (e) {
    ...
    console.log("FIELDS REQUIRED")
        ...
        $.post('https://x-multicharacter/createNewCharacter', JSON.stringify({
            firstname: firstname,
            lastname: lastname,
        ...
        xMultiCharacters.fadeOutDown('.character-register', '125%', 400);
        refreshCharacters()
    }
});
// $(document).on('click', '#create', function(e){
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://x-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://x-multicharacter/selectCharacter', JSON.stringify({
                cData: $(selectedChar).data('cData')
            }));
            // xMultiCharacters.fadeInDown('.welcomescreen', WelcomePercentage, 400);
            // xMultiCharacters.fadeInDown('.server-log', '25%', 400);
            setTimeout(function(){
                xMultiCharacters.fadeOutDown('.characters-list', "-40%", 400);
                xMultiCharacters.fadeOutDown('.character-info', "-40%", 400);
                xMultiCharacters.resetAll();
            }, 1500);
        } else {
            $('.characters-list').css("filter", "blur(2px)")
            $('.character-info').css("filter", "blur(2px)")
            xMultiCharacters.fadeInDown('.character-register', '25%', 400);
        }
    }
});
...
xMultiCharacters.fadeOutUp = function(element, time) {
    $(element).css({"display":"block"}).animate({top: "-80.5%",}, time, function(){
        $(element).css({"display":"none"});
    });
}

xMultiCharacters.fadeOutDown = function(element, percent, time) {
...
xMultiCharacters.fadeInDown = function(element, percent, time) {
    $(element).css({"display":"block"}).animate({top: percent,}, time);
}

xMultiCharacters.resetAll = function() {

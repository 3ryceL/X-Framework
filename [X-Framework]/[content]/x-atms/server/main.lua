...
XCore.Functions.CreateCallback('x-debitcard:server:requestCards', function(source, cb)
    local src = source
...
XCore.Functions.CreateCallback('x-debitcard:server:deleteCard', function(source, cb, data)
...
RegisterCommand('atm', function(source, args, rawCommand)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = XCore.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
    ...
    TriggerClientEvent('x-atms:client:loadATM', src, cards)
end)
...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('XCore:NotifyNotify', -1, "Daily Withdraw Limit Reset", "success")
    end
end)

RegisterServerEvent('x-atms:server:doAccountWithdraw')
AddEventHandler('x-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then 
        local src = source
        local xPlayer = XCore.Functions.GetPlayer(src)
        ...
        if dailyWith < 5000 then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('XCore:NotifyNotify', src, "Withdraw $" .. data.amount .. ' from credit card. Daily Withdraws: ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('XCore:NotifyNotify', src, "You cant go into minus in ATM.", "error")
                end
            ...
            TriggerClientEvent('x-atms:client:updateBankInformation', src, banking)
        else
            TriggerClientEvent('XCore:NotifyNotify', src, "You have reached the daily limit.", "error")
        end
    end
end)


XCore.Functions.CreateCallback('x-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = XCore.Functions.GetPlayer(src)

...
function closeBanking() {
    $.post("https://x-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("https://x-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("https://x-atms/removeCard", JSON.stringify({
    ...
    $(document).on('click','[data-action=ATMwithdraw]',function(){
        ...
        if(amount !== undefined && amount !== null && amount !== 0) {
            $.post("https://x-atms/doATMWithdraw", JSON.stringify({
    ...
    $(document).on('click','[data-act=enterNumber]',function(){
            ...
            if (number == "ACCEPT") {
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("https://x-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {

            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('x-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, 38) then
                                XCore.Functions.TriggerCallback('x-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        XCore.Functions.Notify("You have $250,- deposit returned!")
                                    else
                                        XCore.Functions.Notify("You have no deposit paid on this vehicle..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, 38) then
                                XCore.Functions.TriggerCallback('x-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        XCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            XCore.Functions.Notify("You have $250,- deposit paid!")
                                            XCore.Functions.Notify("You have started working, location marked on GPS!")
                                        end, coords, true)
                                    else
                                        XCore.Functions.Notify("You have not enough money for the deposit.. Deposit costs are $1000,-")
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        if chance < 26 then
                                                            TriggerServerEvent('x-garbagejob:server:nano')
                                                        end
                                                        SetGarbageRoute()
                                                        XCore.Functions.Notify("All garbage bags are done, proceed to the next location!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        XCore.Functions.Notify("You are done working! Go back to the depot.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        XCore.Functions.Notify("There are still "..AmountOfBags.." bags left!")
                                                    else
                                                        XCore.Functions.Notify("There is still "..AmountOfBags.." bags over there!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()

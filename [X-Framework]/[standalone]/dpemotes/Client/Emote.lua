...
AddEventHandler('animations:client:SmokeWeed', function()
  ...
  Citizen.CreateThread(function()
    while SmokingWeed do
      Citizen.Wait(10000)
      TriggerServerEvent('x-hud:Server:RelieveStress', math.random(15, 18))
...
AddEventHandler('animations:client:EmoteCommandStart', function(args)
  if CanDoEmote then
    ...
    XCore.Functions.Notify("Cannot Be Done Right Now", "error")
  end
end)
...
function EmoteCommandStart(source, args, raw)
    ...
    if name == "c" then
        if IsInAnimation then
          ...
          XCore.Functions.Notify('No Emote To Cancel', 'error')
        end
      return
    ...
    elseif DP.PropEmotes[name] ~= nil then
      ...
      XCore.Functions.Notify('That Is Not A Valid Command', 'error')
    end
  end
end
